...
SURY_STRICT_FPR="${SURY_STRICT_FPR:-0}"
APACHE_LOG_DIR="${APACHE_LOG_DIR:-/var/log/apache2}"

SEI_RUN_VERIFY="${SEI_RUN_VERIFY:-1}"
SEI_VERIFY_STRICT="${SEI_VERIFY_STRICT:-1}"
...
init_secrets() {
  ...
  if [[ -n "$SEI_DB_ROOT_PASSWORD" ]]; then
...
relocate_dir_to_optvar() {
  ...
  if [[ -d "$src" ]]; then
    if command -v rsync >/dev/null 2>&1; then
      ...
      log "rsync not found; falling back to cp -a for ${src}"
      cp -a "$src/." "$dst/"
    fi
  fi
...
# -----------------------------
add_sury_repo() {
  log "Adding Sury PHP repo (fingerprint check lenient)"
  wait_for_apt_locks
  apt-get update -y
  ...
  if [[ -n "$got_fpr" && "$got_fpr" != "${SURY_EXPECTED_FPR^^}" ]]; then
    if [[ "$SURY_STRICT_FPR" == "1" ]]; then
      die "Sury key fingerprint mismatch. expected=${SURY_EXPECTED_FPR^^} got=$got_fpr"
    else
      echo "WARN: Sury key fingerprint mismatch (expected ${SURY_EXPECTED_FPR^^} got ${got_fpr}); proceeding because SURY_STRICT_FPR!=1" >&2
    fi
  elif [[ -z "$got_fpr" ]]; then
    echo "WARN: Could not read fingerprint from ${SURY_KEYRING}; proceeding because SURY_STRICT_FPR!=1" >&2
  fi
...
ensure_prefix_layout() {
  ...
  chmod 755 "$PREFIX" "$APP_DIR" "$DATA_DIR" "$OPT_VAR" "$TMPROOT"
...
install_composer_deps() {
  ...
  runuser -u www-data -- env COMPOSER_HOME="$COMPOSER_HOME_DIR" \
...
install_solr() {
  ...
  mapfile -t extracted_dirs < <(find "${TMPROOT}/solr-extract" -maxdepth 1 -type d -name 'solr-*')
  if (( ${#extracted_dirs[@]} != 1 )); then
    die "Expected exactly one solr-* directory in archive (found ${#extracted_dirs[@]})."
  fi
...
summarize() {
  ...
  echo "MariaDB root  : unix_socket by default (no generated password unless provided)"
...
main() {
  ...
  if [[ "$SEI_RUN_VERIFY" == "1" ]]; then
    if ! bash "${SRC_ROOT}/scripts/verify-sei-stack.sh"; then
      if [[ "$SEI_VERIFY_STRICT" == "1" ]]; then
        die "Verification failed and SEI_VERIFY_STRICT=1"
      else
        echo "WARN: Verification failed but continuing because SEI_VERIFY_STRICT!=1" >&2
      fi
    fi
  else
    log "Skipping verifier (SEI_RUN_VERIFY!=1)"
  fi
}
