...
class InfraArray {
  ...
  */
  ...
  public static function ordenarArrInfraDTO(&$arrObjInfraDTO, $strAtributo, $strTipoOrdenacao){
    $arrObjs = array();

    if (count($arrObjInfraDTO)){
      $strPrefixo = $arrObjInfraDTO[0]->getPrefixo($strAtributo);
    }
    ...
    if ($todosNumericos && $peloMenosUmGrande){
      if ($strTipoOrdenacao==InfraArray::$TIPO_ORDENACAO_ASC){
  ...
  public static function sortNumerosGrandes( $arr, $left = 0 , $right = NULL ) {
    ...
    {
      $array = $arr;
      $right = count($array)-1;//ultimo elemento do array
    }
...
// a direita de $tmp ficam os valores maiores
    ...
    // quando todas as particoes tiverem um elemento
  ...
  */
  public static function distinctArrInfraDTO($arrObjInfraDTO, $strAtributo){
    ...
    //contem os valores de strAtributo j? adicionados
   ...
   * @param string $strAtributoChave
   * @param bool $bolAtributoRepetido
   * @return array
   */
  public static function indexarArrInfraDTO($arrObjInfraDTO, $strAtributoChave, $bolAtributoRepetido=false){
    ...
    if (!empty($arrObjInfraDTO)){
      ...
      if (!$bolAtributoRepetido){
        foreach($arrObjInfraDTO as $dto){
          $ret[$dto->get($strAtributoChave)] = $dto;
        }
      }else{
        foreach($arrObjInfraDTO as $dto){
          if (!isset($ret[$dto->get($strAtributoChave)])){
            $ret[$dto->get($strAtributoChave)] = array($dto);
          }else{
            $ret[$dto->get($strAtributoChave)][] = $dto;
          }
        }
  ...
  */
  public static function converterArrInfraDTO($arrObjInfraDTO, $strAtributoValor=null, $strAtributoChave=null, $bolAtributoRepetido=false){

  	$ret = array();

    if ($strAtributoValor==null){
	    ...
	    if (!$bolAtributoRepetido){
		    foreach($arrObjInfraDTO as $dto){
		      if ($strAtributoChave==null){
		        $ret[] = $dto->get($strAtributoValor);
		      }else{
		        $ret[$dto->get($strAtributoChave)] = $dto->get($strAtributoValor);
		      }
		    }
	    }else{
	    	if ($strAtributoChave!=null){
		      foreach($arrObjInfraDTO as $dto){
		        if (!isset($ret[$dto->get($strAtributoChave)])){
		          $ret[$dto->get($strAtributoChave)] = array($dto->get($strAtributoValor));
		        }else{
		          $ret[$dto->get($strAtributoChave)][] = $dto->get($strAtributoValor);
		        }
		      }
...
public static function sumArray($arr, $strAtributoUnico, $arrAtributosValorSoma){
  ...
  foreach($arr as $item){
    ...
    if(!in_array($id,$arrAdicionados)){
                  ...
                  //SE J? ESTIVER NO ARRAY, SOMAR? O VALOR DO REGISTRO DUPLICADO AO PRIMEIRO VALOR SALVO NO DTO DE IDS UNICOS
