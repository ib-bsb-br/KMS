...
abstract class InfraPagina {
  ...
  private static $POS_SEL_NUM_AREA_TABELA = 15;
	...
	public abstract function getStrNomeSistema();
	public abstract function getStrMenuSistema();
	public abstract function getArrStrAcoesBarraSistema();

    /**
     * @return InfraSessao
     */
	public abstract function getObjInfraSessao();
	public abstract function isBolProducao();
  ...
  public function isBolIphone(){
	...
	public static function getNumVersaoFirefox(){
		$strVersao = null;
		$strBrowser = $_SERVER['HTTP_USER_AGENT'];
		$posNavegador = strpos($strBrowser,'Firefox/');
		if ($posNavegador !== false){
			$posFimVersao = strpos($strBrowser,' ',$posNavegador);
			if ($posFimVersao!==false){
				$strVersao = substr($strBrowser,$posNavegador+strlen('Firefox/'),$posFimVersao-($posNavegador+strlen('Firefox/')));
			}else{
				$strVersao = substr($strBrowser,$posNavegador+strlen('Firefox/'));
			}
		}
		return self::formatarVersaoNavegador($strVersao);
	}

	public static function getNumVersaoChrome(){
		$strVersao = null;
		$strBrowser = $_SERVER['HTTP_USER_AGENT'];
		$posNavegador = strpos($strBrowser,'Chrome/');

		if ($posNavegador !== false){
			$posFimVersao = strpos($strBrowser,' ',$posNavegador);
			if ($posFimVersao!==false){
				$strVersao = substr($strBrowser,$posNavegador+strlen('Chrome/'),$posFimVersao-($posNavegador+strlen('Chrome/')));
			}
		}
		return self::formatarVersaoNavegador($strVersao);
	}
	public static function getNumVersaoEdge(){
		$strVersao = null;
		$strBrowser = $_SERVER['HTTP_USER_AGENT'];
		$re = "/Edge\\/(\\d+(.\\d+)*)/";

		if (preg_match($re, $strBrowser, $matches)!=0){
				$strVersao = $matches[1][0];
		}
		return self::formatarVersaoNavegador($strVersao);
	}

	public static function getNumVersaoInternetExplorer(){
		$strVersao = null;
		$strBrowser = $_SERVER['HTTP_USER_AGENT'];
		$posNavegador = strpos($strBrowser,'MSIE ');
		if ($posNavegador !== false){
			$posFimVersao = strpos($strBrowser,';',$posNavegador);
			if ($posFimVersao!==false){
				$strVersao = substr($strBrowser,$posNavegador+strlen('MSIE '),$posFimVersao-($posNavegador+strlen('MSIE ')));
			}
		} else if (strpos($strBrowser,'Trident/')!==false){
			$posNavegador = strpos($strBrowser,'rv:');

      $posFimVersao1 = strpos($strBrowser,';',$posNavegador);
			$posFimVersao2 = strpos($strBrowser,')',$posNavegador);
      $posFimVersao3 = strpos($strBrowser,' ',$posNavegador);

      $numTamUA = strlen($strBrowser);
      ...
      if ($posFimVersao1 < $posFimVersao2 && $posFimVersao1 < $posFimVersao3){
        ...
        $posFimVersao = $posFimVersao3;
			...
			$strVersao=substr($strBrowser,$posNavegador+3,$posFimVersao-($posNavegador+3));
		}
		return self::formatarVersaoNavegador($strVersao);
	}

	public static function getNumVersaoSafariIpad(){
		$strVersao = null;
		$strBrowser = $_SERVER['HTTP_USER_AGENT'];
		...
		if (strpos($strBrowser,'iPad;') !== false){
			$posNavegador = strpos($strBrowser,'Version/');
			if ($posNavegador !== false){
				$posFimVersao = strpos($strBrowser,' ',$posNavegador);
				if ($posFimVersao!==false){
					$strVersao = substr($strBrowser,$posNavegador+strlen('Version/'),$posFimVersao-($posNavegador+strlen('Version/')));
				}
			}
		}
		return self::formatarVersaoNavegador($strVersao);
	...
	public static function getNumVersaoSafari(){
		$strVersao = null;
		$strBrowser = $_SERVER['HTTP_USER_AGENT'];
		...
		if (strpos($strBrowser,'iPad;')===false && strpos($strBrowser,'Chrome/')===false && strpos($strBrowser,'Safari/')!==false){
			$posNavegador = strpos($strBrowser,'Version/');
			if ($posNavegador !== false){

				$strVersao = substr($strBrowser,$posNavegador+strlen('Version/'));
	...
	private static function formatarVersaoNavegador($strVersao){
		if ($strVersao!=null){
			...
			if (isset($arrVersao[0]) && isset($arrVersao[1])){
...
';
		...
		if ($this->adicionarJQuery() && $this->tipoMenu == self::$MENU_SMART){
...
';
	...
	private function montarBarraSistema() {
		if ($this->getTipoPagina()==self::$TIPO_PAGINA_COMPLETA || $this->getTipoPagina()==self::$TIPO_PAGINA_SEM_MENU){
			...
			echo '</div>'."\n".
					...
					'<label>'.$this->getStrNomeSistema().'</label>'."\n".
					'</div>'."\n".
					'</div>'."\n";
		}
	}
  ...
  public function gerarHashConteudo($strItens){
	...
	public function getArrStrItensSelecionados($strNomeSelecao='Infra', $bolManterOrdemMarcacao = true){
		...
		if ($this->validarHashTabelas() && count($ret)) {
...
//          this.container.hide();
          ...
          this.ask(false); /* Last call to inform the server that the client has closed */
        },
          ...
          $.ajax(this.url,{
            ...
            success: (function(data,text,transport) {
              if (this._status == ProgressBar.STATUS_CLOSE_CLIENT)
                this._status = ProgressBar.STATUS_INACTIVE; /* Disable polling */
...
//                      alert('close');
                  ...
                  if (progressArray[name].status != ProgressBar.STATUS_SERVER_CLOSED) {
                    close=false;
									...
									this._BPAtual.progressbar("option",{value: this._progress!==false?this._progress-this._min:false,max: this._max-this._min });
    ...
    $this->abrirJavaScript();
    ?>
       var prb = new ProgressBar('#divInfraBarraProgresso', '<?=$_SERVER['REQUEST_URI']?>', {delay:<?=$delay?>, autoHide: false});
    <?
    $this->fecharJavaScript();
    $this->fecharBody();
    $this->fecharHtml();
  ...
  * @param $strTexto - texto para tratamento
  ...
  */
  ...
  public static function tratarHTML($str, $imprimirViaJavascript=false, $encoding='iso-8859-1'){
    ...
    public function prepararOrdenacao($objInfraDTO, $strCampoOrdDefault, $strTipoOrdDefault, $bolIgnorarSessao = false, $strSelecao = 'Infra'){
		...
		if (isset($_POST['hdn'.$strSelecao.'CampoOrd']) && isset($_POST['hdn'.$strSelecao.'TipoOrd']) &&
    ...
    if ($strIcone==null){
      $strIcone = $this->getDiretorioImagensGlobal().'/usuario.gif';
		...
		return '<a id="lnkUsuarioSistema" title="'.$strDados.'" tabindex="'.$this->getProxTabBarraSistema().'"><img src="'.$strIcone.'" title="'.$strDados.'" alt="'.$strDados.'" class="infraImg" /></a>';
	}
    ...
    foreach ($arrMenu as $arrMenuPrincipal) {
			$numLimite = count($arrMenuPrincipal);
			for ($i=0; $i<$numLimite; $i++) {
