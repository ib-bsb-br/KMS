...
try {
  ...
  if ($numRegistros > 0){
    ...
    foreach(array_keys($arrAtributos) as $nome){
      ...
      if ($bolAcaoAlterar){
        $strResultado .= '<a href="'.SessaoInfra::getInstance()->assinarLink('controlador.php?acao=infra_atributo_sessao_alterar&acao_origem='.$_GET['acao'].'&acao_retorno='.$_GET['acao'].'&nome='.$nome).'" tabindex="'.PaginaInfra::getInstance()->getProxTabTabela().'"><img src="'.PaginaInfra::getInstance()->getDiretorioImagensGlobal().'/alterar.gif" title="Alterar Atributo de Sessão" alt="Alterar Atributo de Sessão" class="infraImg" /></a>&nbsp;';
      }
      ...
      if ($bolAcaoExcluir){
        $strResultado .= '<a href="'.PaginaInfra::getInstance()->montarAncora($strId).'" onclick="acaoExcluir(\''.$strId.'\',\''.$strDescricao.'\');" tabindex="'.PaginaInfra::getInstance()->getProxTabTabela().'"><img src="'.PaginaInfra::getInstance()->getDiretorioImagensGlobal().'/excluir.gif" title="Excluir Atributo de Sessão" alt="Excluir Atributo de Sessão" class="infraImg" /></a>&nbsp;';
      }
