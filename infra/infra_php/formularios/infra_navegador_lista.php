 ...
 */    }else{
    ...
    */
    ...
    for($i = 0;$i < $numRegistros; $i++){
      ...
      if ($bolAcaoConsultar){
        $strResultado .= '<a href="'.SessaoInfra::getInstance()->assinarLink('controlador.php?acao=infra_navegador_consultar&acao_origem='.$_GET['acao'].'&acao_retorno='.$_GET['acao'].'&id_infra_navegador='.$arrObjInfraNavegadorDTO[$i]->getDblIdInfraNavegador())).'" tabindex="'.PaginaInfra::getInstance()->getProxTabTabela().'"><img src="'.PaginaInfra::getInstance()->getDiretorioImagensGlobal().'/consultar.gif" title="Consultar Navegador" alt="Consultar Navegador" class="infraImg" /></a>&nbsp;';
      }

      if ($bolAcaoAlterar){
        $strResultado .= '<a href="'.SessaoInfra::getInstance()->assinarLink('controlador.php?acao=infra_navegador_alterar&acao_origem='.$_GET['acao'].'&acao_retorno='.$_GET['acao'].'&id_infra_navegador='.$arrObjInfraNavegadorDTO[$i]->getDblIdInfraNavegador())).'" tabindex="'.PaginaInfra::getInstance()->getProxTabTabela().'"><img src="'.PaginaInfra::getInstance()->getDiretorioImagensGlobal().'/alterar.gif" title="Alterar Navegador" alt="Alterar Navegador" class="infraImg" /></a>&nbsp;';
      }
      ...
      if ($bolAcaoDesativar){
        $strResultado .= '<a href="'.PaginaInfra::getInstance()->montarAncora($strId).'" onclick="acaoDesativar(\''.$strId.'\',\''.$strDescricao.'\');" tabindex="'.PaginaInfra::getInstance()->getProxTabTabela().'"><img src="'.PaginaInfra::getInstance()->getDiretorioImagensGlobal().'/desativar.gif" title="Desativar Navegador" alt="Desativar Navegador" class="infraImg" /></a>&nbsp;';
      }

      if ($bolAcaoReativar){
        $strResultado .= '<a href="'.PaginaInfra::getInstance()->montarAncora($strId).'" onclick="acaoReativar(\''.$strId.'\',\''.$strDescricao.'\');" tabindex="'.PaginaInfra::getInstance()->getProxTabTabela().'"><img src="'.PaginaInfra::getInstance()->getDiretorioImagensGlobal().'/reativar.gif" title="Reativar Navegador" alt="Reativar Navegador" class="infraImg" /></a>&nbsp;';
      }
 

      if ($bolAcaoExcluir){
        $strResultado .= '<a href="'.PaginaInfra::getInstance()->montarAncora($strId).'" onclick="acaoExcluir(\''.$strId.'\',\''.$strDescricao.'\');" tabindex="'.PaginaInfra::getInstance()->getProxTabTabela().'"><img src="'.PaginaInfra::getInstance()->getDiretorioImagensGlobal().'/excluir.gif" title="Excluir Navegador" alt="Excluir Navegador" class="infraImg" /></a>&nbsp;';
      }
...
<form id="frmInfraNavegadorLista" method="post" onsubmit="return validarForm();" action="<?=SessaoInfra::getInstance()->assinarLink('controlador.php?acao='.$_GET['acao'].'&acao_origem='.$_GET['acao'])?>">
  ...
  <img src="<?=PaginaInfra::getInstance()->getDiretorioImagensGlobal()?>/calendario.gif" id="imgCalDthInicialNavegadores" title="Selecionar Data/Hora Inicial" alt="Selecionar Data/Hora Inicial" class="infraImg" onclick="infraCalendario('txtDthInicialNavegadores', this, true,'<?=InfraData::getStrDataAtual().' 00:00'?>');" />
  ...
  <img src="<?=PaginaInfra::getInstance()->getDiretorioImagensGlobal()?>/calendario.gif" id="imgCalDthFinalNavegadores" title="Selecionar Data/Hora Final" alt="Selecionar Data/Hora Final" class="infraImg" onclick="infraCalendario('txtDthFinalNavegadores', this, true, '<?=InfraData::getStrDataAtual().' 23:59'?>');" />
