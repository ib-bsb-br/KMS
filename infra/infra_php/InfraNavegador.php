...
abstract class InfraNavegador extends InfraRN {
  ...
  protected function registrarControlado() {
		try {
	      ...
	      $objInfraSequencia = new InfraSequencia($this->objInfraIBanco);
	  		$numProxSeq = $objInfraSequencia->obterProximaSequencia('infra_navegador');
		  }else if ($this->getNumTipoPK()==InfraDTO::$TIPO_PK_NATIVA){
  		  $numProxSeq = $this->getObjInfraIBanco()->getValorSequencia('seq_infra_navegador');
		  }else{
      ...
      //Mozilla/5.0 (Windows NT 6.2; ARM; Trident/7.0; Touch; rv:11.0; WPDesktop; Lumia 1320) like Gecko
  		...
  		$strVersao = InfraPagina::getNumVersaoInternetExplorer();

  		if ($strVersao != null){
  		  $strIdentificacao = self::$TN_INTERNET_EXPLORER;
  		}else{

    		$strVersao = InfraPagina::getNumVersaoFirefox();
    		...
    		if ($strVersao != null){
    		  ...
    		  $strIdentificacao = self::$TN_FIREFOX;
					...
					$strVersao = InfraPagina::getNumVersaoEdge();
					...
					if ($strVersao != null){
						...
						$strVersao = InfraPagina::getNumVersaoChrome();
						...
						if ($strVersao != null){
							...
							$strVersao = InfraPagina::getNumVersaoSafariIpad();
							...
							if ($strVersao != null){
								...
								$strVersao = InfraPagina::getNumVersaoSafari();
  	  ...
  	  $sql = '';
  	  $sql .= ' INSERT INTO infra_navegador (id_infra_navegador, dth_acesso, ip, identificacao, versao, user_agent) ';
  	  $sql .= ' VALUES (';
  	  $sql .= $numProxSeq.',';
  	  $sql .= $this->objInfraIBanco->formatarGravacaoDth(InfraData::getStrDataHoraAtual()).',';
  	  $sql .= $this->objInfraIBanco->formatarGravacaoStr(InfraUtil::getStrIpUsuario()).',';
  	  $sql .= $this->objInfraIBanco->formatarGravacaoStr($strIdentificacao).',';
  	  $sql .= $this->objInfraIBanco->formatarGravacaoStr($strVersao).',';
  	  $sql .= $this->objInfraIBanco->formatarGravacaoStr($_SERVER['HTTP_USER_AGENT']).')';
