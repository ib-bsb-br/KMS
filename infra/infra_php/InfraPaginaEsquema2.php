  ...
  public function getArquivoCssEsquemaN(){
	...
	private function montarBarraSuperior(){
	  if ($this->getTipoPagina()==self::$TIPO_PAGINA_COMPLETA || $this->getTipoPagina()==self::$TIPO_PAGINA_SEM_MENU){
  	  ...
  	  if ($this->getStrTextoBarraSuperior()!=null){
  	    echo '<label>'.InfraString::transformarCaixaAlta($this->getStrTextoBarraSuperior()).'</label>'."\n";
  	  }else{
  	    if ($this->getObjInfraSessao()!=null){
  	      echo '<label>'.InfraString::transformarCaixaAlta($this->getObjInfraSessao()->getStrDescricaoOrgaoSistema()).'</label>'."\n";
  	    }
 	  	}
      echo '</div>'."\n";
	  }
	...
	private function montarBarraSistema() {
      ...
      if ($this->getStrTextoBarraSistema()!=null){
        echo '<label>'.$this->getStrTextoBarraSistema().'</label>';
      }else{
  	    if ($this->getStrLogoSistema()!==null){
          echo $this->getStrLogoSistema()."\n";
        }else{
          if ($this->getObjInfraSessao()!=null){
            echo '<label>'.$this->getObjInfraSessao()->getStrSiglaSistema().'</label>';
          }
        }
      }
	  	  echo '</div>'."\n".
