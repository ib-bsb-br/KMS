  ...
  if ($numRegistros > 0){
    ...
    for($i = 0;$i < $numRegistros; $i++){
      ...
      $strResultado .= '<td align="center">';
			...
			//if ($bolAcaoConsultar){
      //  $strResultado .= '<a href="'.SessaoSip::getInstance()->assinarLink('controlador.php?acao=menu_consultar&id_menu='.$arrObjMenuDTO[$i]->getNumIdMenu())).'" tabindex="'.PaginaSip::getInstance()->getProxTabDados().'"><img src="imagens/consultar.gif" title="Consultar Menu" alt="Consultar Menu" class="infraImg" /></a>&nbsp;';
      //}

      if ($bolAcaoListarItens){
        $strResultado .= '<a href="'.SessaoSip::getInstance()->assinarLink('controlador.php?acao=item_menu_listar&acao_retorno=menu_listar&id_menu='.$arrObjMenuDTO[$i]->getNumIdMenu().'&id_sistema='.$arrObjMenuDTO[$i]->getNumIdSistema().'&id_orgao_sistema='.$arrObjMenuDTO[$i]->getNumIdOrgaoSistema()).'" tabindex="'.PaginaSip::getInstance()->getProxTabDados().'"><img src="imagens/menu.gif" title="Listar Itens de Menu" alt="Listar Itens de Menu" class="infraImg" /></a>&nbsp;';
      }
			
      if ($bolAcaoAlterar){
        $strResultado .= '<a href="'.SessaoSip::getInstance()->assinarLink('controlador.php?acao=menu_alterar&id_menu='.$arrObjMenuDTO[$i]->getNumIdMenu()).'" tabindex="'.PaginaSip::getInstance()->getProxTabDados().'"><img src="imagens/alterar.gif" title="Alterar Menu" alt="Alterar Menu" class="infraImg" /></a>&nbsp;';
      }
      ...
      if ($bolAcaoDesativar){
        $strResultado .= '<a onclick="acaoDesativar(\''.$strId.'\',\''.$strDescricao.'\');" tabindex="'.PaginaSip::getInstance()->getProxTabDados().'"><img src="imagens/desativar.gif" title="Desativar Menu" alt="Desativar Menu" class="infraImg" /></a>&nbsp;';
      }

      if ($bolAcaoExcluir){
        $strResultado .= '<a onclick="acaoExcluir(\''.$strId.'\',\''.$strDescricao.'\');" tabindex="'.PaginaSip::getInstance()->getProxTabDados().'"><img src="imagens/excluir.gif" title="Excluir Menu" alt="Excluir Menu" class="infraImg" /></a>&nbsp;';
      }
