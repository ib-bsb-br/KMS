  ...
  $objItemMenuDTO = new ItemMenuDTO();
    ...
    if ($numRegistros > 0){
      ...
      for($i = 0;$i < $numRegistros; $i++){
        ...
        if ($bolAcaoCadastrar){
          $strResultado .= '<a href="'.SessaoSip::getInstance()->assinarLink('controlador.php?acao=item_menu_cadastrar&acao_origem=item_menu_listar&id_orgao_sistema='.$numIdOrgao.'&id_sistema='.$arrObjItemMenuDTO[$i]->getNumIdSistema().'&id_menu_superior='.$arrObjItemMenuDTO[$i]->getNumIdMenu().'&id_item_menu_superior='.$arrObjItemMenuDTO[$i]->getNumIdItemMenu()).'" tabindex="'.PaginaSip::getInstance()->getProxTabDados().'"><img src="imagens/mais.gif" title="Adicionar Subitem" alt="Adicionar Subitem" class="infraImg" /></a>&nbsp;';
        }
        
        if ($bolAcaoAlterar){
          $strResultado .= '<a href="'.SessaoSip::getInstance()->assinarLink('controlador.php?acao=item_menu_alterar&&acao_origem=item_menu_listar&acao_retorno=item_menu_listar&id_menu='.$arrObjItemMenuDTO[$i]->getNumIdMenu().'&id_item_menu='.$arrObjItemMenuDTO[$i]->getNumIdItemMenu()).'" tabindex="'.PaginaSip::getInstance()->getProxTabDados().'"><img src="imagens/alterar.gif" title="Alterar Item de Menu" alt="Alterar Item de Menu" class="infraImg" /></a>&nbsp;';
        }
        
        if ($bolAcaoDesativar){
          $strResultado .= '<a onclick="acaoDesativar(\''.$strId.'\',\''.$strDescricao.'\');" tabindex="'.PaginaSip::getInstance()->getProxTabDados().'"><img src="imagens/desativar.gif" title="Desativar Item de Menu" alt="Desativar Item de Menu" class="infraImg" /></a>&nbsp;';
        }
        
        if ($bolAcaoExcluir){
          $strResultado .= '<a onclick="acaoExcluir(\''.$strId.'\',\''.$strDescricao.'\');" tabindex="'.PaginaSip::getInstance()->getProxTabDados().'"><img src="imagens/excluir.gif" title="Excluir Item de Menu" alt="Excluir Item de Menu" class="infraImg" /></a>&nbsp;';
        }
