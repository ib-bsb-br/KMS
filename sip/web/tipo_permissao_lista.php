  ...
  switch($_GET['acao']){
	...
	if ($numRegistros > 0){
		...
		for($i = 0;$i < $numRegistros; $i++){
			...
			//if ($bolAcaoConsultar){
			//  $strResultado .= '<a href="'.SessaoSip::getInstance()->assinarLink('controlador.php?acao=tipo_permissao_consultar&id_tipo_permissao='.$arrObjTipoPermissaoDTO[$i]->getNumIdTipoPermissao())).'" tabindex="'.PaginaSip::getInstance()->getProxTabDados().'"><img src="imagens/consultar.gif" title="Consultar Tipo de Permissão" alt="Consultar Tipo de Permissão" class="infraImg" /></a>&nbsp;';
			//}

			if ($bolAcaoAlterar){
				$strResultado .= '<a href="'.SessaoSip::getInstance()->assinarLink('controlador.php?acao=tipo_permissao_alterar&id_tipo_permissao='.$arrObjTipoPermissaoDTO[$i]->getNumIdTipoPermissao()).'" tabindex="'.PaginaSip::getInstance()->getProxTabDados().'"><img src="imagens/alterar.gif" title="Alterar Tipo de Permissão" alt="Alterar Tipo de Permissão" class="infraImg" /></a>&nbsp;';
			}

			if ($bolAcaoExcluir){
				$strResultado .= '<a onclick="acaoExcluir(\''.$arrObjTipoPermissaoDTO[$i]->getNumIdTipoPermissao().'\',\''.PaginaSip::formatarParametrosJavaScript($arrObjTipoPermissaoDTO[$i]->getStrDescricao()).'\');" tabindex="'.PaginaSip::getInstance()->getProxTabDados().'"><img src="imagens/excluir.gif" title="Excluir Tipo de Permissão" alt="Excluir Tipo de Permissão" class="infraImg" /></a>&nbsp;';
			}
