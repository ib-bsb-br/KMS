    ...
    if ($_GET['acao']=='orgao_selecionar'){
      ...
      $bolAcaoDesativar = SessaoSip::getInstance()->verificarPermissao('orgao_desativar');
		...
		if ($bolAcaoExcluir){
			...
			$strLinkExcluir = SessaoSip::getInstance()->assinarLink('controlador.php?acao=orgao_excluir');
		}

		if ($bolAcaoDesativar){
			$bolCheck = true;
			//$arrComandos[] = '<input type="button" id="btnDesativar" value="Desativar" onclick="acaoDesativacaoMultipla();" class="infraButton" />';
			$strLinkDesativar = SessaoSip::getInstance()->assinarLink('controlador.php?acao=orgao_desativar');
		}

		if ($bolAcaoReativar){
			$bolCheck = true;
			//$arrComandos[] = '<input type="button" id="btnDesativar" value="Desativar" onclick="acaoDesativacaoMultipla();" class="infraButton" />';
			$strLinkReativar = SessaoSip::getInstance()->assinarLink('controlador.php?acao=orgao_reativar');
		}
    ...
    $strResultado .= '<th class="infraTh" width="10%">'.PaginaSip::getInstance()->getThOrdenacao($objOrgaoDTO,'Ordem','Ordem',$arrObjOrgaoDTO).'</th>';
      ...
      $strResultado .= '<td align="center">'.PaginaSip::tratarHTML($arrObjOrgaoDTO[$i]->getNumOrdem()).'</td>';
		  ...
		  $strResultado .= '<td align="center">';
			if ($bolAcaoConsultar){
				$strResultado .= '<a href="'.SessaoSip::getInstance()->assinarLink('controlador.php?acao=orgao_consultar&id_orgao='.$arrObjOrgaoDTO[$i]->getNumIdOrgao()).'" tabindex="'.PaginaSip::getInstance()->getProxTabDados().'"><img src="imagens/consultar.gif" title="Consultar Órgão" alt="Consultar Órgão" class="infraImg" /></a>&nbsp;';
			}

			if ($bolAcaoAlterar){
				$strResultado .= '<a href="'.SessaoSip::getInstance()->assinarLink('controlador.php?acao=orgao_alterar&id_orgao='.$arrObjOrgaoDTO[$i]->getNumIdOrgao()).'" tabindex="'.PaginaSip::getInstance()->getProxTabDados().'"><img src="imagens/alterar.gif" title="Alterar Órgão" alt="Alterar Órgão" class="infraImg" /></a>&nbsp;';
			}
      ...
      if ($bolAcaoDesativar || $bolAcaoReativar || $bolAcaoExcluir){
        ...
        $strDescricao = $arrObjOrgaoDTO[$i]->getStrSigla();
			...
			if ($bolAcaoDesativar){
				$strResultado .= '<a onclick="acaoDesativar(\''.$strId.'\',\''.$strDescricao.'\');" tabindex="'.PaginaSip::getInstance()->getProxTabDados().'"><img src="imagens/desativar.gif" title="Desativar Órgão" alt="Desativar Órgão" class="infraImg" /></a>&nbsp;';
			}
			
			if ($bolAcaoReativar){
				$strResultado .= '<a onclick="acaoReativar(\''.$strId.'\',\''.$strDescricao.'\');" tabindex="'.PaginaSip::getInstance()->getProxTabDados().'"><img src="imagens/reativar.gif" title="Reativar Órgão" alt="Reativar Órgão" class="infraImg" /></a>&nbsp;';
			}
			
			if ($bolAcaoExcluir){
				$strResultado .= '<a onclick="acaoExcluir(\''.$strId.'\',\''.$strDescricao.'\');" tabindex="'.PaginaSip::getInstance()->getProxTabDados().'"><img src="imagens/excluir.gif" title="Excluir Órgão" alt="Excluir Órgão" class="infraImg" /></a>&nbsp;';
			}
