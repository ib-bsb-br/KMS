...
try {
  ...
  switch($_GET['acao']){
	...
	if ($numRegistros > 0){
		...
		for($i = 0;$i < $numRegistros; $i++){
			...
			/*
			if ($bolAcaoConsultar){
				$strResultado .= '<a href="'.SessaoSip::getInstance()->assinarLink('controlador.php?acao=coordenador_unidade_consultar&id_sistema='.$arrObjCoordenadorUnidadeDTO[$i]->getNumIdSistema().'&id_usuario='.$arrObjCoordenadorUnidadeDTO[$i]->getNumIdUsuario().'&id_unidade='.$arrObjCoordenadorUnidadeDTO[$i]->getNumIdUnidade())).'" tabindex="'.PaginaSip::getInstance()->getProxTabDados().'"><img src="imagens/consultar.gif" title="Consultar Usuário com Permissão" alt="Consultar Usuário com Permissão" class="infraImg" /></a>&nbsp;';
			}

			if ($bolAcaoAlterar){
				$strResultado .= '<a href="'.SessaoSip::getInstance()->assinarLink('controlador.php?acao=coordenador_unidade_alterar&id_sistema='.$arrObjCoordenadorUnidadeDTO[$i]->getNumIdSistema().'&id_usuario='.$arrObjCoordenadorUnidadeDTO[$i]->getNumIdUsuario().'&id_unidade='.$arrObjCoordenadorUnidadeDTO[$i]->getNumIdUnidade())).'" tabindex="'.PaginaSip::getInstance()->getProxTabDados().'"><img src="imagens/alterar.gif" title="Alterar Usuário com Permissão" alt="Alterar Usuário com Permissão" class="infraImg" /></a>&nbsp;';
			}
      */
			
			if ($bolAcaoExcluir){
				$strResultado .= '<a onclick="acaoExcluir(\''.$arrObjCoordenadorUnidadeDTO[$i]->getNumIdSistema().'#'.$arrObjCoordenadorUnidadeDTO[$i]->getNumIdUsuario().'#'.$arrObjCoordenadorUnidadeDTO[$i]->getNumIdUnidade().'\',\''.PaginaSip::formatarParametrosJavaScript($arrObjCoordenadorUnidadeDTO[$i]->getStrSiglaUnidade().'/'.$arrObjCoordenadorUnidadeDTO[$i]->getStrSiglaUsuario().'/'.$arrObjCoordenadorUnidadeDTO[$i]->getStrSiglaSistema()).'\');" tabindex="'.PaginaSip::getInstance()->getProxTabDados().'"><img src="imagens/excluir.gif" title="Excluir Coordenador de Unidade" alt="Excluir Coordenador de Unidade" class="infraImg" /></a>&nbsp;';
			}
  ...
  $strItensSelOrgaoUnidade = OrgaoINT::montarSelectSiglaAutorizados('','Todos', $numIdOrgaoUnidade);
