...
try {
  ...
  if ($numRegistros > 0){
    ...
    for($i = 0;$i < $numRegistros; $i++){
      ...
      if ($bolAcaoConsultar){
        $strResultado .= '<a href="'.SessaoSip::getInstance()->assinarLink('controlador.php?acao=recurso_consultar&acao_origem='.$_GET['acao'].'&acao_retorno='.$_GET['acao'].'&id_sistema='.$arrObjRecursoDTO[$i]->getNumIdSistema().'&id_recurso='.$arrObjRecursoDTO[$i]->getNumIdRecurso()).'" tabindex="'.PaginaSip::getInstance()->getProxTabTabela().'"><img src="'.PaginaSip::getInstance()->getDiretorioImagensGlobal().'/consultar.gif" title="Consultar Recurso" alt="Consultar Recurso" class="infraImg" /></a>&nbsp;';
      }

      if ($bolAcaoAlterar){
        $strResultado .= '<a href="'.SessaoSip::getInstance()->assinarLink('controlador.php?acao=recurso_alterar&acao_origem='.$_GET['acao'].'&acao_retorno='.$_GET['acao'].'&id_sistema='.$arrObjRecursoDTO[$i]->getNumIdSistema().'&id_recurso='.$arrObjRecursoDTO[$i]->getNumIdRecurso()).'" tabindex="'.PaginaSip::getInstance()->getProxTabTabela().'"><img src="'.PaginaSip::getInstance()->getDiretorioImagensGlobal().'/alterar.gif" title="Alterar Recurso" alt="Alterar Recurso" class="infraImg" /></a>&nbsp;';
      }
      ...
      if ($bolAcaoDesativar){
        $strResultado .= '<a href="#ID-'.$strId.'" onclick="acaoDesativar(\''.$strId.'\',\''.$strDescricao.'\');" tabindex="'.PaginaSip::getInstance()->getProxTabTabela().'"><img src="'.PaginaSip::getInstance()->getDiretorioImagensGlobal().'/desativar.gif" title="Desativar Recurso" alt="Desativar Recurso" class="infraImg" /></a>&nbsp;';
      }

      if ($bolAcaoReativar){
        $strResultado .= '<a href="#ID-'.$strId.'" onclick="acaoReativar(\''.$strId.'\',\''.$strDescricao.'\');" tabindex="'.PaginaSip::getInstance()->getProxTabTabela().'"><img src="'.PaginaSip::getInstance()->getDiretorioImagensGlobal().'/reativar.gif" title="Reativar Recurso" alt="Reativar Recurso" class="infraImg" /></a>&nbsp;';
      }


      if ($bolAcaoExcluir){
        $strResultado .= '<a href="#ID-'.$strId.'" onclick="acaoExcluir(\''.$strId.'\',\''.$strDescricao.'\');" tabindex="'.PaginaSip::getInstance()->getProxTabTabela().'"><img src="'.PaginaSip::getInstance()->getDiretorioImagensGlobal().'/excluir.gif" title="Excluir Recurso" alt="Excluir Recurso" class="infraImg" /></a>&nbsp;';
      }
...
#lblNomeRecurso {position:absolute;left:33%;top:0%;width:50%;}
#txtNomeRecurso {position:absolute;left:33%;top:20%;width:50%;}
