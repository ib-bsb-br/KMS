	...
	if ($numRegistros > 0){
      ...
      if ($bolAcaoCopiar || $bolAcaoExcluir){
        ...
        $strDescricao = PaginaSip::formatarParametrosJavaScript($arrObjPermissaoDTO[$i]->getStrSiglaUsuario().' / '.$arrObjPermissaoDTO[$i]->getStrSiglaSistema().' / '.$arrObjPermissaoDTO[$i]->getStrNomePerfil());
			...
			if ($bolAcaoCopiar){
				$strResultado .= '<a onclick="acaoCopiar(\''.$strId.'\',\''.$strDescricao.'\');" tabindex="'.PaginaSip::getInstance()->getProxTabDados().'"><img src="imagens/copiar.gif" title="Copiar Permissão" alt="Copiar Permissão" class="infraImg" /></a>&nbsp;';
			}
			
			if ($bolAcaoConsultar){
				$strResultado .= '<a href="'.SessaoSip::getInstance()->assinarLink('controlador.php?acao=permissao_consultar&acao_origem=permissao_listar_administradas&acao_retorno=permissao_listar_administradas&id_perfil='.$arrObjPermissaoDTO[$i]->getNumIdPerfil().'&id_sistema='.$arrObjPermissaoDTO[$i]->getNumIdSistema().'&id_usuario='.$arrObjPermissaoDTO[$i]->getNumIdUsuario().'&id_unidade='.$arrObjPermissaoDTO[$i]->getNumIdUnidade()).'" tabindex="'.PaginaSip::getInstance()->getProxTabDados().'"><img src="imagens/consultar.gif" title="Consultar Permissão" alt="Consultar Permissão" class="infraImg" /></a>&nbsp;';
			}

			if ($bolAcaoAlterar){
				$strResultado .= '<a href="'.SessaoSip::getInstance()->assinarLink('controlador.php?acao=permissao_alterar&acao_origem=permissao_listar_administradas&acao_retorno=permissao_listar_administradas&id_perfil='.$arrObjPermissaoDTO[$i]->getNumIdPerfil().'&id_sistema='.$arrObjPermissaoDTO[$i]->getNumIdSistema().'&id_usuario='.$arrObjPermissaoDTO[$i]->getNumIdUsuario().'&id_unidade='.$arrObjPermissaoDTO[$i]->getNumIdUnidade()).'" tabindex="'.PaginaSip::getInstance()->getProxTabDados().'"><img src="imagens/alterar.gif" title="Alterar Permissão" alt="Alterar Permissão" class="infraImg" /></a>&nbsp;';
			}

			if ($bolAcaoExcluir){
				$strResultado .= '<a onclick="acaoExcluir(\''.$strId.'\',\''.$strDescricao.'\');" tabindex="'.PaginaSip::getInstance()->getProxTabDados().'"><img src="imagens/excluir.gif" title="Excluir Permissão" alt="Excluir Permissão" class="infraImg" /></a>&nbsp;';
			}
...
function inicializar(){
  ...
  objAjaxUsuario.selecionar('<?=$numIdUsuario;?>','<?=PaginaSip::getInstance()->formatarParametrosJavascript($strSiglaUsuario);?>','<?=PaginaSip::getInstance()->formatarParametrosJavascript($strNomeUsuario)?>');
