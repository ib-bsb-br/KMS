    ...
    try{
        ...
        $ret = null;
    		...
    		if ($objUsuarioDTO!=null){
    		  ...
    		  $objRelGrupoRedeUnidadeDTO->setStrOuLdapGrupoRede($ret['ContextoUnidade']);
    		  ...
    		  if ($objRelGrupoRedeUnidadeDTO==null){
    		    ...
    		    $ret['IdUnidadePadrao'] = $objRelGrupoRedeUnidadeDTO->getNumIdUnidade();
    		  }
  ...
  protected function copiarControlado(PermissaoCopiarDTO $objPermissaoCopiarDTO) {
    try{
      ...
      $objInfraException = new InfraException();
			...
			//Verifica se o usuario já possui a permissao na unidade selecionada
			$arrStrPermissoesValidacao = array();
			foreach($objPermissaoCopiarDTO->getArrObjPermissaoDTO() as $objPermissaoDTO){
				...
				if ($objPermissaoCopiarDTO->getNumIdUnidade()==null){
					...
					$numIdUnidade = $objPermissaoDTO->getNumIdUnidade();
					$strSiglaOrgaoUnidade = $objPermissaoDTO->getStrSiglaUnidade();
					$strSiglaUnidade = $objPermissaoDTO->getStrSiglaOrgaoUnidade();
					...
					$numIdUnidade = $objPermissaoCopiarDTO->getNumIdUnidade();
					$strSiglaOrgaoUnidade = $objUnidadeDTO->getStrSiglaOrgao();
					$strSiglaUnidade = $objUnidadeDTO->getStrSigla();

					$dto->setNumIdUnidade($objPermissaoCopiarDTO->getNumIdUnidade());
				}

				$strChave = $objPermissaoDTO->getNumIdSistema().'#'.$objPermissaoDTO->getNumIdUsuario().'#'.$objPermissaoDTO->getNumIdPerfil().'#'.$numIdUnidade;
				if (!in_array($strChave,$arrStrPermissoesValidacao)) {
					if ($this->consultar($dto) != null) {
						$objInfraException->adicionarValidacao('Usuário já possui permissão no sistema ' . $objPermissaoDTO->getStrSiglaSistema() . '/' . $objPermissaoDTO->getStrSiglaOrgaoSistema() . ', no perfil "' . $objPermissaoDTO->getStrNomePerfil() . '" e unidade ' . $strSiglaUnidade . '/' . $strSiglaOrgaoUnidade . '.');
						$arrStrPermissoesValidacao[] = $strChave;
					}
				}
			 }
     ...
     foreach($objPermissaoCopiarDTO->getArrObjPermissaoDTO() as $objPermissaoDTO){
			  ...
			  if ($objPermissaoCopiarDTO->getNumIdUnidade()==null){
				...
				if (!in_array($strChave,$arrStrPermissoesCopiadas)){
					//Cadastra copia
