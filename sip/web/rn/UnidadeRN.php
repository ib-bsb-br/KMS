      ...
      $objInfraException = new InfraException();
			for($i=0;$i<count($arrObjUnidadeDTO);$i++){
			  ...
			  $dto = $this->consultar($dto);
				...
				$objRelGrupoRedeUnidadeDTO->setNumIdUnidade($arrObjUnidadeDTO[$i]->getNumIdUnidade());
				$objRelGrupoRedeUnidadeRN = new RelGrupoRedeUnidadeRN();
				if ($objRelGrupoRedeUnidadeRN->contar($objRelGrupoRedeUnidadeDTO)>0){
          $objInfraException->adicionarValidacao('A unidade '.$dto->getStrSigla().' possui grupo de rede associado e não pode ser excluída.');				  
				}
