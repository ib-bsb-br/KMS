...
try {
  ...
  $numProtocolos = count($arrObjRelProtocoloProtocoloDTO);
  ...
  if ($numProtocolos) {
    ...
    foreach ($SEI_MODULOS as $seiModulo) {
      ...
      if (count($arrObjDocumentoAPIAutorizado)) {
      ...
      if (count($arrObjDocumentoAPINegado)) {
      ...
      if (count($arrObjProcedimentoAPIAutorizado)) {
      ...
      if (count($arrObjProcedimentoAPINegado)) {
    ...
    foreach ($arrObjRelProtocoloProtocoloDTO as $objRelProtocoloProtocoloDTO) {
      ...
      if ($objRelProtocoloProtocoloDTO->getStrStaAssociacao() == RelProtocoloProtocoloRN::$TA_DOCUMENTO_ASSOCIADO) {
        ...
        if ($objRelProtocoloProtocoloDTO->getStrSinAcessoExterno()=='S'){
          $strLinkDocumento .= ' class="ancoraPadraoAzul" onclick="window.open(\'' . SessaoSEIExterna::getInstance()->assinarLink('documento_consulta_externa.php?id_acesso_externo=' . $_GET['id_acesso_externo'] . '&id_documento=' . $objDocumentoDTO->getDblIdDocumento()) . '\');"';
          ...
          $strLinkDocumento .= ' class="ancoraPadraoPreta" onclick="alert(\'Documento cancelado.\')" style="text-decoration: line-through"';
        }else{
          $strLinkDocumento .= ' class="ancoraPadraoPreta" onclick="alert(\'Sem acesso ao documento.\')"';
        }
        ...
        if ($objRelProtocoloProtocoloDTO->getStrSinAcessoExterno()=='S'){
          $strLinkProcessoAnexado .= ' class="ancoraPadraoAzul" onclick="window.open(\'' . SessaoSEIExterna::getInstance()->assinarLink('processo_acesso_externo_consulta.php?id_acesso_externo=' . $_GET['id_acesso_externo'] . '&id_procedimento_anexado=' . $objProcedimentoDTOAnexado->getDblIdProcedimento()) . '\');"';
        }else{
          $strLinkProcessoAnexado .= ' class="ancoraPadraoPreta" onclick="alert(\'Sem acesso ao processo anexado.\')"';
        }
  ...
  if ($_POST['hdnFlagGerar']=='1' || $_POST['hdnFlagGerar']=='2'){
    ...
    }else if ($strSiglaUsuarioExterno==null){
      $strSiglaUsuarioExterno = $objAcessoExternoDTO->getStrNomeContato();
    }else{
      $strSiglaUsuarioExterno = 'Externo';
    }
