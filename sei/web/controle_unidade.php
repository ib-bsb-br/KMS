    ...
    if ($numRegistros >0){
      ...
      for($i = 0;$i < $numRegistros; $i++){
        ...
        if ($bolAcaoAndamentoSituacaoGerenciar) {
          $strResultado .= '<a href="' . SessaoSEI::getInstance()->assinarLink('controlador.php?acao=andamento_situacao_gerenciar&acao_origem=' . $_GET['acao'] . '&acao_retorno=' . $_GET['acao'] . '&id_procedimento=' . $arrObjAndamentoSituacaoDTO[$i]->getDblIdProcedimento()) . '" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="' . PaginaSEI::getInstance()->getDiretorioImagensLocal() . '/sei_situacao_pequeno.png" title="Gerenciar Ponto de Controle" alt="Gerenciar Ponto de Controle" class="infraImg" /></a>&nbsp;';
        }
...
#divSinSituacoesDesativadas {position:absolute;left:66%;top:50%;}
...
function abrirDetalhe(link){
 infraAbrirJanela(link,'janelaAcompanhamentoUnidadeDetalhe',850,550,'location=0,status=1,resizable=1,scrollbars=1');
}
...
<form id="frmAcompanhamentoUnidade"  method="post" onsubmit="return onSubmitForm();" action="<?=SessaoSEI::getInstance()->assinarLink('controlador.php?acao='.$_GET['acao'].'&acao_origem='.$_GET['acao'])?>">
  ...
  PaginaSEI::getInstance()->abrirAreaDados('5em','style="overflow:visible;"');
  ?>
