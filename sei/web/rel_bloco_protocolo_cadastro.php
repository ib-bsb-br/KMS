...
try {
    ...
    case 'rel_bloco_protocolo_alterar':
      ...
      if ($_GET['acao_origem']=='rel_bloco_protocolo_listar'){
        ...
        if ($objRelBlocoProtocoloDTO==null){
          ...
          PaginaSEI::getInstance()->setStrMensagem("Documento foi retirado do bloco.", InfraPagina::$TIPO_MSG_AVISO);
      ...
      if (isset($_POST['sbmAlterarRelBlocoProtocolo'])) {
        try{
          ...
          $strLinkRetorno = SessaoSEI::getInstance()->assinarLink('controlador.php?acao='.PaginaSEI::getInstance()->getAcaoRetorno().'&acao_origem='.$_GET['acao'].$strParametros.PaginaSEI::getInstance()->montarAncora($objRelBlocoProtocoloDTO->getDblIdProtocolo().'-'.$objRelBlocoProtocoloDTO->getNumIdBloco()));
          //die;
  ...
  //$strItensSelProtocolo = ProtocoloINT::montarSelect???????('null','&nbsp;',$objRelBlocoProtocoloDTO->getDblIdProtocolo());
  //$strItensSelBloco = BlocoINT::montarSelect???????('null','&nbsp;',$objRelBlocoProtocoloDTO->getNumIdBloco());
...
PaginaSEI::getInstance()->abrirAreaDados('23em');
?>
  <!-- <label id="lblAnotacao" for="txtAnotacao" accesskey="" class="infraLabelObrigatorio">Anotações:</label> -->
	<textarea id="txtAnotacao" name="txtAnotacao" rows="<?=PaginaSEI::getInstance()->isBolNavegadorFirefox()?'12':'13'?>" class="infraTextArea" tabindex="<?=PaginaSEI::getInstance()->getProxTabDados()?>"><?=PaginaSEI::tratarHTML($objRelBlocoProtocoloDTO->getStrAnotacao());?></textarea>
