...
try {
  ...
  if ($objEstatisticasDTORet != null){
    ...
    $strResultadoDesempenho .= '<caption class="infraCaption">'.PaginaSEI::getInstance()->gerarCaptionTabela(EstatisticasRN::$TITULO_DESEMPENHO,count($arrObjEstatisticasTabelaDESEMPENHO)).'</caption>';
...
#lblPeriodoA 	{position:absolute;left:26%;top:64%;width:9%;}
...
<form id="frmEstatisticasDesempenho" onsubmit="return validarFormulario();" method="post" action="<?=SessaoSEI::getInstance()->assinarLink('controlador.php?acao='.$_GET['acao'].'&acao_origem='.$_GET['acao'])?>">
  ...
  PaginaSEI::getInstance()->abrirAreaDados('19em');
  ?>
