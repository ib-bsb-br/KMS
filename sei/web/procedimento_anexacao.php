  ...
  if ($numRegistros > 0){
    ...
    for($i = 0;$i < $numRegistros; $i++){
      ...
      if($bolAcaoDesanexarProcedimento){
        $strResultado .= '<a href="'.SessaoSEI::getInstance()->assinarLink('controlador.php?acao=procedimento_desanexar&acao_origem='.$_GET['acao'].'&acao_retorno='.$_GET['acao'].$strParametros.'&id_procedimento_anexado='.$arrObjRelProtocoloProtocoloDTO[$i]->getDblIdProtocolo2()).'" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="imagens/remover_vinculo_procedimento.gif" title="Desanexar Processo" alt="Desanexar Processo" class="infraImg" /></a>&nbsp;';
      }
...
#btnPesquisar {position:absolute;left:26%;top:35%;width:10%;}
