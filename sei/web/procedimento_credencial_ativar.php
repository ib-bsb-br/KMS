...
try {
  ...
  switch($_GET['acao']){
    ...
    case 'procedimento_credencial_ativar':
      ...
      if ($_GET['acao_origem']=='procedimento_acervo_sigilosos'){
      ...
      if ($_POST['sbmSalvar']=='Salvar') {
        try {
          ...
          $bolExecutouOK = true;
          $strLinkRetorno = SessaoSEI::getInstance()->assinarLink('controlador.php?acao=procedimento_acervo_sigilosos&acao_origem='.$_GET['acao'].$strParametros.PaginaSEI::montarAncora($arrIdProcedimento));
...
<?if($bolExecutouOK){?>
  var frmOpener = window.opener.document.getElementById('frmProcedimentoAcervoSigilosos');
