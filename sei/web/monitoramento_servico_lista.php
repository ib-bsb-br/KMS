 ...
 */
  ...
  $numRegistros = count($arrObjMonitoramentoServicoDTO);
    ...
    */
    ...
    for($i = 0;$i < $numRegistros; $i++){
      ...
      if ($objMonitoramentoServicoDTO->getStrStaTipo()==MonitoramentoServicoRN::$TM_DETALHADO) {
        ...
        if ($bolAcaoConsultar) {
          $strResultado .= '<a href="' . SessaoSEI::getInstance()->assinarLink('controlador.php?acao=monitoramento_servico_consultar&acao_origem=' . $_GET['acao'] . '&acao_retorno=' . $_GET['acao'] . '&id_monitoramento_servico=' . $arrObjMonitoramentoServicoDTO[$i]->getDblIdMonitoramentoServico()) . '" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="' . PaginaSEI::getInstance()->getDiretorioImagensGlobal() . '/consultar.gif" title="Consultar Monitoramento de Serviço" alt="Consultar Monitoramento de Serviço" class="infraImg" /></a>&nbsp;';
        }

        if ($bolAcaoAlterar) {
          $strResultado .= '<a href="' . SessaoSEI::getInstance()->assinarLink('controlador.php?acao=monitoramento_servico_alterar&acao_origem=' . $_GET['acao'] . '&acao_retorno=' . $_GET['acao'] . '&id_monitoramento_servico=' . $arrObjMonitoramentoServicoDTO[$i]->getDblIdMonitoramentoServico()) . '" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="' . PaginaSEI::getInstance()->getDiretorioImagensGlobal() . '/alterar.gif" title="Alterar Monitoramento de Serviço" alt="Alterar Monitoramento de Serviço" class="infraImg" /></a>&nbsp;';
        }
        ...
        /*
              if ($bolAcaoDesativar){
                $strResultado .= '<a href="'.PaginaSEI::getInstance()->montarAncora($strId).'" onclick="acaoDesativar(\''.$strId.'\',\''.$strDescricao.'\');" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="'.PaginaSEI::getInstance()->getDiretorioImagensGlobal().'/desativar.gif" title="Desativar Monitoramento de Serviço" alt="Desativar Monitoramento de Serviço" class="infraImg" /></a>&nbsp;';
              }

              if ($bolAcaoReativar){
                $strResultado .= '<a href="'.PaginaSEI::getInstance()->montarAncora($strId).'" onclick="acaoReativar(\''.$strId.'\',\''.$strDescricao.'\');" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="'.PaginaSEI::getInstance()->getDiretorioImagensGlobal().'/reativar.gif" title="Reativar Monitoramento de Serviço" alt="Reativar Monitoramento de Serviço" class="infraImg" /></a>&nbsp;';
              }
         */

        if ($bolAcaoExcluir) {
          $strResultado .= '<a href="' . PaginaSEI::getInstance()->montarAncora($strId) . '" onclick="acaoExcluir(\'' . $strId . '\',\'' . $strDescricao . '\');" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="' . PaginaSEI::getInstance()->getDiretorioImagensGlobal() . '/excluir.gif" title="Excluir Monitoramento de Serviço" alt="Excluir Monitoramento de Serviço" class="infraImg" /></a>&nbsp;';
        }
...
<form id="frmMonitoramentoServicoLista" method="post" onsubmit="return onSubmitForm();" action="<?=SessaoSEI::getInstance()->assinarLink('controlador.php?acao='.$_GET['acao'].'&acao_origem='.$_GET['acao'])?>">
  ...
  <img src="<?=PaginaSEI::getInstance()->getDiretorioImagensGlobal()?>/calendario.gif" id="imgCalDthInicial" title="Selecionar Data/Hora Inicial" alt="Selecionar Data/Hora Inicial" class="infraImg" onclick="infraCalendario('txtDthInicialMon',this,true,'<?=InfraData::getStrDataAtual().' 00:00'?>');" tabindex="<?=PaginaSEI::getInstance()->getProxTabDados()?>" />
  ...
  <img src="<?=PaginaSEI::getInstance()->getDiretorioImagensGlobal()?>/calendario.gif" id="imgCalDthFinal" title="Selecionar Data/Hora Final" alt="Selecionar Data/Hora Final" class="infraImg" onclick="infraCalendario('txtDthFinalMon',this,true,'<?=InfraData::getStrDataAtual().' 23:59'?>');" tabindex="<?=PaginaSEI::getInstance()->getProxTabDados()?>" />
