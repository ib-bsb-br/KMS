 ...
 */    }else{
    ...
    for($i = 0;$i < $numRegistros; $i++){
      ...
      if ($bolAcaoConsultar){
        $strResultado .= '<a href="'.SessaoSEI::getInstance()->assinarLink('controlador.php?acao=controle_interno_consultar&acao_origem='.$_GET['acao'].'&acao_retorno='.$_GET['acao'].'&id_controle_interno='.$arrObjControleInternoDTO[$i]->getNumIdControleInterno()).'" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="'.PaginaSEI::getInstance()->getDiretorioImagensGlobal().'/consultar.gif" title="Consultar Controle Interno" alt="Consultar Controle Interno" class="infraImgNormal" /></a>&nbsp;';
      }

      if ($bolAcaoAlterar){
        $strResultado .= '<a href="'.SessaoSEI::getInstance()->assinarLink('controlador.php?acao=controle_interno_alterar&acao_origem='.$_GET['acao'].'&acao_retorno='.$_GET['acao'].'&id_controle_interno='.$arrObjControleInternoDTO[$i]->getNumIdControleInterno()).'" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="'.PaginaSEI::getInstance()->getDiretorioImagensGlobal().'/alterar.gif" title="Alterar Controle Interno" alt="Alterar Controle Interno" class="infraImgNormal" /></a>&nbsp;';
      }
...
/* 
      if ($bolAcaoDesativar){
        $strResultado .= '<a href="'.PaginaSEI::getInstance()->montarAncora($strId).'" onclick="acaoDesativar(\''.$strId.'\',\''.$strDescricao.'\');" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="'.PaginaSEI::getInstance()->getDiretorioImagensGlobal().'/desativar.gif" title="Desativar Controle Interno" alt="Desativar Controle Interno" class="infraImgNormal" /></a>&nbsp;';
      }

      if ($bolAcaoReativar){
        $strResultado .= '<a href="'.PaginaSEI::getInstance()->montarAncora($strId).'" onclick="acaoReativar(\''.$strId.'\',\''.$strDescricao.'\');" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="'.PaginaSEI::getInstance()->getDiretorioImagensGlobal().'/reativar.gif" title="Reativar Controle Interno" alt="Reativar Controle Interno" class="infraImgNormal" /></a>&nbsp;';
      }
 */

      if ($bolAcaoExcluir){
        $strResultado .= '<a href="'.PaginaSEI::getInstance()->montarAncora($strId).'" onclick="acaoExcluir(\''.$strId.'\',\''.$strDescricao.'\');" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="'.PaginaSEI::getInstance()->getDiretorioImagensGlobal().'/excluir.gif" title="Excluir Controle Interno" alt="Excluir Controle Interno" class="infraImgNormal" /></a>&nbsp;';
      }
...
function abrirJanelaExclusao(){
  ...
  infraAbrirJanela('<?=$strLinkExcluir?>','janelaExcluirCriterio',600,150,'location=0,status=1,resizable=1,scrollbars=1');

  var frm = document.getElementById('frmControleInternoLista');
  var actionAnterior = frm.action;
  frm.target = 'janelaExcluirCriterio';
