...
try {
  ...
  switch($_GET['acao']){
    ...
    case 'grupo_protocolo_modelo_selecionar':
      $strTitulo = PaginaSEI::getInstance()->getTituloSelecao('Selecionar Grupo de Modelo','Selecionar Grupo de Modelos');
  ...
  if ($numRegistros > 0){
    ...
    /* if ($_GET['acao']!='grupo_protocolo_modelo_reativar'){ */
      $strSumarioTabela = 'Tabela de Grupo de Modelos.';
      $strCaptionTabela = 'Grupo de Modelos';
    /* }else{
      $strSumarioTabela = 'Tabela de Grupo de Modelos Inativos.';
      $strCaptionTabela = 'Grupo de Modelos Inativos';
    } */
    ...
    for($i = 0;$i < $numRegistros; $i++){
      ...
      if ($bolAcaoConsultar){
        $strResultado .= '<a href="'.SessaoSEI::getInstance()->assinarLink('controlador.php?acao=grupo_protocolo_modelo_consultar&acao_origem='.$_GET['acao'].'&acao_retorno='.$_GET['acao'].'&id_grupo_protocolo_modelo='.$arrObjGrupoProtocoloModeloDTO[$i]->getNumIdGrupoProtocoloModelo()).'" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="'.PaginaSEI::getInstance()->getDiretorioImagensGlobal().'/consultar.gif" title="Consultar Grupo de Modelo" alt="Consultar Grupo de Modelo" class="infraImg" /></a>&nbsp;';
      }

      if ($bolAcaoAlterar){
        $strResultado .= '<a href="'.SessaoSEI::getInstance()->assinarLink('controlador.php?acao=grupo_protocolo_modelo_alterar&acao_origem='.$_GET['acao'].'&acao_retorno='.$_GET['acao'].'&id_grupo_protocolo_modelo='.$arrObjGrupoProtocoloModeloDTO[$i]->getNumIdGrupoProtocoloModelo()).'" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="'.PaginaSEI::getInstance()->getDiretorioImagensGlobal().'/alterar.gif" title="Alterar Grupo de Modelo" alt="Alterar Grupo de Modelo" class="infraImg" /></a>&nbsp;';
      }
      ...
      if ($bolAcaoExcluir){
        $strResultado .= '<a href="'.PaginaSEI::getInstance()->montarAncora($strId).'" onclick="acaoExcluir(\''.$strId.'\',\''.$strDescricao.'\');" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="'.PaginaSEI::getInstance()->getDiretorioImagensGlobal().'/excluir.gif" title="Excluir Grupo de Modelo" alt="Excluir Grupo de Modelo" class="infraImg" /></a>&nbsp;';
      }
...
function acaoDesativar(id,desc){
  if (confirm("Confirma desativação do Grupo de Modelo \""+desc+"\"?")){
...
function acaoDesativacaoMultipla(){
  if (document.getElementById('hdnInfraItensSelecionados').value==''){
    alert('Nenhum Grupo de Modelo selecionado.');
    return;
  }
  if (confirm("Confirma desativação dos Grupo de Modelos selecionados?")){
...
function acaoReativar(id,desc){
  if (confirm("Confirma reativação do Grupo de Modelo \""+desc+"\"?")){
    document.getElementById('hdnInfraItemId').value=id;
...
function acaoReativacaoMultipla(){
  ...
  if (confirm("Confirma reativação dos Grupo de Modelos selecionados?")){
    document.getElementById('hdnInfraItemId').value='';
    document.getElementById('frmGrupoProtocoloModeloLista').action='<?=$strLinkReativar?>';
    document.getElementById('frmGrupoProtocoloModeloLista').submit();
  }
...
function acaoExcluir(id,desc){
  if (confirm("Confirma exclusão do Grupo de Modelo \""+desc+"\"?")){
    document.getElementById('hdnInfraItemId').value=id;
...
function acaoExclusaoMultipla(){
  ...
  if (confirm("Confirma exclusão dos Grupo de Modelos selecionados?")){
    document.getElementById('hdnInfraItemId').value='';
    document.getElementById('frmGrupoProtocoloModeloLista').action='<?=$strLinkExcluir?>';
    document.getElementById('frmGrupoProtocoloModeloLista').submit();
  }
