  ...
  if ($numRegistros > 0){
    ...
    for($i = 0;$i < $numRegistros; $i++){
      ...
      if ($bolAcaoLiberar && $arrObjBaseConhecimentoDTO[$i]->getStrStaEstado()==BaseConhecimentoRN::$TE_RASCUNHO){
      	$bolFlagLiberar = true;
        $strResultado .= '<a href="#ID-'.$strId.'"  onclick="acaoLiberar(\''.$strId.'\',\''.$strDescricao.'\');" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="'.PaginaSEI::getInstance()->getDiretorioImagensGlobal().'/marcar.gif" title="Liberar Versão" alt="Liberar Versão" class="infraImg" /></a>&nbsp;';
      }
      ...
      if ($bolAcaoCancelarLiberacao && $arrObjBaseConhecimentoDTO[$i]->getStrStaEstado()==BaseConhecimentoRN::$TE_LIBERADO){
        ...
        $strResultado .= '<a href="#ID-'.$strId.'"  onclick="acaoCancelarLiberacao(\''.$strId.'\',\''.$strDescricao.'\');" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="'.PaginaSEI::getInstance()->getDiretorioImagensGlobal().'/menos.gif" title="Cancelar Liberação de Versão" alt="Cancelar Liberação de Versão" class="infraImg" /></a>&nbsp;';
      }

      if ($bolVisualizarVersoes && $arrObjBaseConhecimentoDTO[$i]->getNumIdBaseConhecimentoOrigem() != null){
      	$strResultado .= '<a href="'.SessaoSEI::getInstance()->assinarLink('controlador.php?acao=base_conhecimento_versoes&id_base_conhecimento_agrupador='.$arrObjBaseConhecimentoDTO[$i]->getNumIdBaseConhecimentoAgrupador().'&id_base_conhecimento='.$arrObjBaseConhecimentoDTO[$i]->getNumIdBaseConhecimento().'&acao_origem='.$_GET['acao'].'&acao_retorno='.$_GET['acao']).'" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="'.PaginaSEI::getInstance()->getDiretorioImagensGlobal().'/versoes.gif" title="Versões do Procedimento" alt="Versões do Procedimento" class="infraImg" /></a>&nbsp;';
      }
      ...
      if ($bolAcaoExcluir && $arrObjBaseConhecimentoDTO[$i]->getNumIdUnidade()==SessaoSEI::getInstance()->getNumIdUnidadeAtual() && $arrObjBaseConhecimentoDTO[$i]->getStrStaEstado()==BaseConhecimentoRN::$TE_RASCUNHO){
      	$bolFlagExcluir = true;
        $strResultado .= '<a href="#ID-'.$strId.'"  onclick="acaoExcluir(\''.$strId.'\',\''.$strDescricao.'\');" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="'.PaginaSEI::getInstance()->getDiretorioImagensGlobal().'/excluir.gif" title="Excluir Procedimento" alt="Excluir Procedimento" class="infraImg" /></a>&nbsp;';
      }
