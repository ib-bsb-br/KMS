...
try {
  ...
  if ($numRegistros > 0){
    ...
    for($i = 0;$i < $numRegistros; $i++){
      ...
      if($bolAcaoRemoverSobrestamento){
        $strResultado .= '<a href="'.PaginaSEI::getInstance()->montarAncora($arrObjProcessoSobrestadoDTO[$i]->getDblIdProcedimento()).'" onclick="acaoRemoverSobrestamento(\''.$arrObjProcessoSobrestadoDTO[$i]->getDblIdProcedimento().'\',\''.$arrObjProcessoSobrestadoDTO[$i]->getStrProtocoloProcedimentoFormatado().'\');" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="imagens/sei_remover_sobrestamento_processo_pequeno.gif" title="Remover Sobrestamento" alt="Remover Sobrestamento" class="infraImg" /></a>&nbsp;';
      }
