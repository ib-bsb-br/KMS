    ...
    case 'procedimento_gerar_zip':
      ...
      if (count($objProcedimentoDTO->getArrObjDocumentoDTO())){
        ...
        foreach($objProcedimentoDTO->getArrObjDocumentoDTO() as $objDocumentoDTO){
          ...
          if(isset($arrObjProtocoloDTO[$objDocumentoDTO->getDblIdDocumento()]) && $objDocumentoRN->verificarSelecaoGeracaoZip($objDocumentoDTO)){
...
function inicializar(){   
  ...
  <?if ($bolGeracaoOK){ ?>  
    window.open('<?=SessaoSEI::getInstance()->assinarLink('controlador.php?acao=exibir_arquivo&nome_arquivo='.$objAnexoDTO->getStrNome().'&nome_download=SEI-'.$objProcedimentoDTO->getStrProtocoloProcedimentoFormatado().'.zip&acao_origem='.$_GET['acao'].'&acao_retorno='.$_GET['acao']);?>');
  <?}?>
