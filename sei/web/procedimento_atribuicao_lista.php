...
try {
  ...
  if ($numRegistros > 0){
    ...
    for($i = 0;$i < $numRegistros; $i++){
      ...
      $strResultado .= '<td align="center">';
      if ($bolAcaoAlterar){
        $strResultado .= '<a href="#ID-'.$arrObjProcedimentoDTO[$i]->getDblIdProcedimento().'"  onclick="acaoAlterar(\''.$arrObjProcedimentoDTO[$i]->getDblIdProcedimento().'\',\''.$arrObjProcedimentoDTO[$i]->getStrProtocoloProcedimentoFormatado().'\');" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="imagens/alterar.gif" title="Alterar Atribuição" alt="Alterar Atribuição" class="infraImg" /></a>&nbsp;';
      }
      $strResultado .= '</td>'."\n";
      */
