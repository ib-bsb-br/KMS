  ...
  if ($numRegistros > 0){
    ...
    for($i = 0;$i < $numRegistros; $i++){
      ...
      if ($bolAcaoAtivar && $arrObjTabelaAssuntosDTO[$i]->getStrSinAtual()=='N'){
        $strResultado .= '<a href="'.PaginaSEI::getInstance()->montarAncora($strId).'" onclick="acaoAtivar(\''.$strDescricao.'\',\''.SessaoSEI::getInstance()->assinarLink('controlador.php?acao=tabela_assuntos_ativar&acao_origem='.$_GET['acao'].'&id_tabela_assuntos='.$strId).'\');" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="'.PaginaSEI::getInstance()->getDiretorioImagensLocal().'/ativar.png" title="Ativar Tabela de Assuntos" alt="Ativar Tabela de Assuntos" class="infraImg" /></a>&nbsp;';
      }

      if ($bolAcaoMapeamentoAssuntoListar && $arrObjTabelaAssuntosDTO[$i]->getStrSinAtual()=='S'){
        $strResultado .= '<a href="'.SessaoSEI::getInstance()->assinarLink('controlador.php?acao=mapeamento_assunto_listar&acao_origem='.$_GET['acao'].'&acao_retorno='.$_GET['acao'].'&id_tabela_assuntos_origem='.$arrObjTabelaAssuntosDTO[$i]->getNumIdTabelaAssuntos()).'" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="'.PaginaSEI::getInstance()->getDiretorioImagensLocal().'/mapeamento.png" title="Mapeamentos" alt="Mapeamentos" class="infraImg" /></a>&nbsp;';
      }

      if ($bolAcaoAssuntoListar){
        $strResultado .= '<a href="'.SessaoSEI::getInstance()->assinarLink('controlador.php?acao=assunto_listar&acao_origem='.$_GET['acao'].'&acao_retorno='.$_GET['acao'].'&id_tabela_assuntos='.$arrObjTabelaAssuntosDTO[$i]->getNumIdTabelaAssuntos()).'" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="'.PaginaSEI::getInstance()->getDiretorioImagensGlobal().'/valores.gif" title="Assuntos da Tabela" alt="Assuntos da Tabela" class="infraImg" /></a>&nbsp;';
      }

      if ($bolAcaoConsultar){
        $strResultado .= '<a href="'.SessaoSEI::getInstance()->assinarLink('controlador.php?acao=tabela_assuntos_consultar&acao_origem='.$_GET['acao'].'&acao_retorno='.$_GET['acao'].'&id_tabela_assuntos='.$arrObjTabelaAssuntosDTO[$i]->getNumIdTabelaAssuntos()).'" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="'.PaginaSEI::getInstance()->getDiretorioImagensGlobal().'/consultar.gif" title="Consultar Tabela de Assuntos" alt="Consultar Tabela de Assuntos" class="infraImg" /></a>&nbsp;';
      }

      if ($bolAcaoAlterar){
        $strResultado .= '<a href="'.SessaoSEI::getInstance()->assinarLink('controlador.php?acao=tabela_assuntos_alterar&acao_origem='.$_GET['acao'].'&acao_retorno='.$_GET['acao'].'&id_tabela_assuntos='.$arrObjTabelaAssuntosDTO[$i]->getNumIdTabelaAssuntos()).'" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="'.PaginaSEI::getInstance()->getDiretorioImagensGlobal().'/alterar.gif" title="Alterar Tabela de Assuntos" alt="Alterar Tabela de Assuntos" class="infraImg" /></a>&nbsp;';
      }

      if ($bolAcaoExcluir){
        $strResultado .= '<a href="'.PaginaSEI::getInstance()->montarAncora($strId).'" onclick="acaoExcluir(\''.$strId.'\',\''.$strDescricao.'\');" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="'.PaginaSEI::getInstance()->getDiretorioImagensGlobal().'/excluir.gif" title="Excluir Tabela de Assuntos" alt="Excluir Tabela de Assuntos" class="infraImg" /></a>&nbsp;';
      }
