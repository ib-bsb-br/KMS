...
class AnexoINT extends InfraINT {
  ...
  public static function montarAnexos($arrIdAnexos, $bolAcaoDownload, $strAcaoDownload, &$arrAcoesDownload, $bolAcaoRemoverAnexo, &$arrAcoesRemover){
    ...
    }else if ($arrIdAnexos!=null && count($arrIdAnexos)){
    ...
    foreach($arrObjAnexoDTO as $objAnexoDTO){
      ...
      if ($bolAcaoDownload && is_numeric($objAnexoDTO->getNumIdAnexo())){
        $arrAcoesDownload[$objAnexoDTO->getNumIdAnexo()] = '<a href="'.SessaoSEI::getInstance()->assinarLink('controlador.php?acao='.$strAcaoDownload.'&id_anexo='.$objAnexoDTO->getNumIdAnexo()).'" target="_blank"><img src="imagens/download.gif" title="Baixar Anexo" alt="Baixar Anexo" class="infraImg" /></a> ';
      }
