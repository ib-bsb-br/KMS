...
try {
  ...
  switch($_GET['acao']){
    ...
    case 'procedimento_alterar':    	
      ...
      if (!isset($_POST['hdnIdProcedimento'])){
        ...
        $objProtocoloDTO->setNumIdTipoProcedimentoProcedimento($_POST['hdnIdTipoProcedimento']);
        $objProcedimentoDTO->setStrNomeTipoProcedimento($_POST['hdnNomeTipoProcedimento']);  
        $objProcedimentoDTO->setStrProtocoloProcedimentoFormatado($_POST['hdnProtocoloProcedimentoFormatado']);
  ...
  if (!isset($_POST['hdnFlagProcedimentoCadastro'])){
    if ($_GET['acao']=='procedimento_gerar' || $_GET['acao']=='procedimento_gerar_relacionado'){
      ...
      if ($_GET['acao']=='procedimento_alterar'){
        ...
        $strAssuntosNegados = implode(',',InfraArray::converterArrInfraDTO($arrObjRelProtocoloAssuntoDTO,'IdAssunto'));
      }
    }
  ...
  $bolHabilitarNumeroProcessoInformado = 0;
  $strDisplayProtocoloInformar = 'display:none;';
  $strExibirProtocoloParaDigitacao = 'visibility:hidden;';
  $strDisplayProtocoloExibir = 'display:none;';
  $bolMarcarProtocoloAutomatico = false;
  $bolMarcarProtocoloManual = false;
  if ($_GET['acao']=='procedimento_gerar' || $_GET['acao']=='procedimento_gerar_relacionado'){
    if (($bolHabilitarNumeroProcessoInformado = $objProcedimentoRN->verificarLiberacaoNumeroProcesso())){
      $strDisplayProtocoloInformar = '';
...
#divProtocoloInformar {<?=$strDisplayProtocoloInformar?>}
...
#divProtocoloExibir {<?=$strDisplayProtocoloExibir?>}
...
function inicializar(){
  ...
  if ('<?=$_GET['acao']?>'=='procedimento_consultar'){
    ...
    infraDesabilitarCamposDiv(document.getElementById('divProtocoloInformar'));
  ...
  <?if ($_GET['acao']=='procedimento_alterar'){?>
    objLupaAssuntos.processarRemocao = function(itens){
      for(var i=0;i < itens.length;i++){
        if (infraInArray(itens[i].value,[<?=$strAssuntosNegados?>])){
          alert('Assunto \"' + itens[i].text + '\" não pode ser removido porque não foi adicionado por esta unidade.');
          return false;
        }
      }
      return true;
  ...
  objLupaInteressados.processarRemocao = function(itens){
    for(var i=0;i < itens.length;i++){
      if (infraInArray(itens[i].value,[<?=$strInteressadosNegados?>])){
        alert('Interessado \"' + itens[i].text + '\" não pode ser removido porque não foi adicionado por esta unidade.');
        return false;
      }
    }
    return true;
...
function validarCadastroRI0152() {
  ...
  <?if ($bolHabilitarNumeroProcessoInformado){?>
...
?>
  ...
  <div id="divProtocoloInformar" class="infraAreaDados" style="height:9em;">
    ...
    <fieldset id="fldProtocoloInformar" class="infraFieldset">
        ...
        <div id="divOptProtocoloAutomatico" class="infraDivRadio"> 
  	    ...
  	    <input type="text" id="txtProtocoloInformar" name="txtProtocoloInformar"  <?=(InfraString::isBolVazia($strMascaraProtocolo)?'':'onkeypress="return infraMascara(this,event,\''.$strMascaraProtocolo.'\');"')?> class="infraText" value="<?=PaginaSEI::tratarHTML($objProcedimentoDTO->getStrProtocoloProcedimentoFormatado());?>" tabindex="<?=PaginaSEI::getInstance()->getProxTabDados()?>" <?=($_POST['rdoProtocolo']=='M'?'style="visibility:visible"':'')?> />
  ...
  <div id="divProtocoloExibir" class="infraAreaDados" style="height:4.5em;">
    ...
    <input type="text" id="txtProtocoloExibir" name="txtProtocoloExibir" class="infraText infraReadOnly" readonly="readonly" value="<?=PaginaSEI::tratarHTML($objProcedimentoDTO->getStrProtocoloProcedimentoFormatado());?>" tabindex="<?=PaginaSEI::getInstance()->getProxTabDados()?>" />
  ...
  <input type="hidden" id="hdnProtocoloProcedimentoFormatado" name="hdnProtocoloProcedimentoFormatado" value="<?=$objProcedimentoDTO->getStrProtocoloProcedimentoFormatado();?>" />
