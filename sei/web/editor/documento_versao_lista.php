...
try {
  ...
  if ($arrObjProtocoloDTO==null) {
    PaginaSEI::getInstance()->setStrMensagem('Acesso negado ao documento.');
    header('Location: '.SessaoSEI::getInstance()->assinarLink('controlador.php?acao=procedimento_controlar&acao_origem=procedimento_controlar'.$strParametros));
    die;    
  }
  ...
  if ($numRegistros > 0){
    ...
    for($i=0;$i < $numRegistros; $i++){
      ...
      if ($bolAcaoVisualizar && $i==0 || ($arrObjVersaoSecaoDocumentoDTO[$i]->getNumIdUnidade()==SessaoSEI::getInstance()->getNumIdUnidadeAtual() && $arrObjProtocoloDTO[0]->getStrSinPublicado()=='N')){
        if ($arrObjProtocoloDTO[0]->getStrStaNivelAcessoGlobal()!=ProtocoloRN::$NA_SIGILOSO || $arrObjVersaoSecaoDocumentoDTO[$i]->getNumIdUsuario()==SessaoSEI::getInstance()->getNumIdUsuario()) {
          $strResultado .= '<a target="_blank" href="'.SessaoSEI::getInstance()->assinarLink('controlador.php?acao=documento_visualizar&acao_origem='.$_GET['acao'].'&acao_retorno='.$_GET['acao'].'&versao='.$arrObjVersaoSecaoDocumentoDTO[$i]->getNumVersao().'&id_documento='.$arrObjVersaoSecaoDocumentoDTO[$i]->getDblIdDocumentoSecaoDocumento().$strParametros).'" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="'.PaginaSEI::getInstance()->getDiretorioImagens().'/sei_documento_interno.gif" title="Visualizar Versão" alt="Visualizar Versão" class="infraImg" /></a>&nbsp;';
        }
      }
      
      if ($i>0 && $bolAcaoRecuperar && $bolFlagAberto && $arrObjVersaoSecaoDocumentoDTO[$i]->getNumIdUnidade()==SessaoSEI::getInstance()->getNumIdUnidadeAtual() && $arrObjProtocoloDTO[0]->getStrSinPublicado()=='N'){
        if ($arrObjProtocoloDTO[0]->getStrStaNivelAcessoGlobal()!=ProtocoloRN::$NA_SIGILOSO || $arrObjVersaoSecaoDocumentoDTO[$i]->getNumIdUsuario()==SessaoSEI::getInstance()->getNumIdUsuario()) {
           $strResultado .= '<a href="#ID-'.$arrObjVersaoSecaoDocumentoDTO[$i]->getNumVersao().'" onclick="acaoRecuperar(\''.$arrObjVersaoSecaoDocumentoDTO[$i]->getNumVersao().'\',\''.$arrObjProtocoloDTO[0]->getStrSinAssinado().'\',\''.SessaoSEI::getInstance()->assinarLink('controlador.php?acao=documento_versao_recuperar&acao_origem='.$_GET['acao'].'&acao_retorno='.$_GET['acao'].'&versao='.$arrObjVersaoSecaoDocumentoDTO[$i]->getNumVersao().'&id_documento='.$arrObjVersaoSecaoDocumentoDTO[$i]->getDblIdDocumentoSecaoDocumento().$strParametros).'\');" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="'.PaginaSEI::getInstance()->getDiretorioImagens().'/sei_recuperar_versao.gif" title="Recuperar Versão" alt="Recuperar Versão" class="infraImg" /></a>&nbsp;';
        }
      }
