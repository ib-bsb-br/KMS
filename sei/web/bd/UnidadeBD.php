...
class UnidadeBD extends InfraBD {
  ...
  public function migrar(MigracaoUnidadeDTO $objMigracaoUnidadeDTO){
    try {
      ...
      $strTextoMigrado = '[Migrado '.$objUnidadeDTOOrigem->getStrSigla().'] ';
...
//        usleep($numRefreshEtapa);
      ...
      if ($objMigracaoUnidadeDTO->getStrSinModelo()=='S') {
        ...
        $prb->setStrRotulo('Migrando Modelos Favoritos...');
        $prb->setNumPosicao(0);
        usleep($numRefreshEtapa);
        ...
        foreach($arrObjProtocoloModeloDTO as $objProtocoloModeloDTO){
          ...
          $prb->setStrRotulo('Modelo Favorito ' . $objProtocoloModeloDTO->getStrProtocoloFormatado() . '...');
          $prb->moverProximo();
          usleep($numRefresh);
