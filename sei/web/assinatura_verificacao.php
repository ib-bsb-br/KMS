      ...
      if ($numRegistros > 0){
        ...
        for($i = 0;$i < $numRegistros; $i++){
          ...
          if ($bolAcaoAssinaturaDownloadP7s && $arrObjAssinaturaDTO[$i]->getStrStaFormaAutenticacao()==AssinaturaRN::$TA_CERTIFICADO_DIGITAL){
            $strResultado .= '<a href="'.SessaoSEI::getInstance()->assinarLink('controlador.php?acao=assinatura_download_p7s&acao_origem='.$_GET['acao'].'&acao_retorno='.$_GET['acao'].'&id_assinatura='.$arrObjAssinaturaDTO[$i]->getNumIdAssinatura()).'" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'" target="_blank"><img src="imagens/download.gif" title="Baixar arquivo PKCS #7 para validação" alt="Baixar arquivo PKCS #7 para validação" class="infraImg" /></a>&nbsp;';
          }
