...
function inicializar(){
 ...
 objAutoCompletarInteressado.selecionar('<?=$strIdInteressado?>','<?=PaginaSEI::getInstance()->formatarParametrosJavaScript($strNomeInteressado)?>');
...
<form id="frmProcedimentoDuplicar" method="post" onsubmit="return OnSubmitForm();" action="<?=SessaoSEI::getInstance()->assinarLink('controlador.php?acao='.$_GET['acao'].'&acao_origem='.$_GET['acao'].'&acao_retorno='.PaginaSEI::getInstance()->getAcaoRetorno().$strParametros)?>">
   ...
   <div class="infraAreaDados" style="height:7em">
     ...
     <img id="imgPesquisarInteressados" onclick="objLupaInteressados.selecionar(700,500);" src="/infra_css/imagens/lupa.gif" alt="Pesquisar Interessados" title="Pesquisar Interessados" class="infraImg" tabindex="<?=PaginaSEI::getInstance()->getProxTabDados()?>" />
