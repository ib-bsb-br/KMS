...
try {
  ...
  if ($numRegistros > 0){
    ...
    for($i = 0;$i < $numRegistros; $i++){
      ...
      if ($bolCheck){
        $strResultado .= '<td valign="top">'.PaginaSEI::getInstance()->getTrCheck($i,$arrObjGrupoAcompanhamentoDTO[$i]->getNumIdGrupoAcompanhamento(),$arrObjGrupoAcompanhamentoDTO[$i]->getNumIdGrupoAcompanhamento()).'</td>';
      }
      ...
      if ($bolAcaoConsultar){
        $strResultado .= '<a href="'.SessaoSEI::getInstance()->assinarLink('controlador.php?acao=grupo_acompanhamento_consultar&acao_origem='.$_GET['acao'].'&acao_retorno='.$_GET['acao'].'&id_grupo_acompanhamento='.$arrObjGrupoAcompanhamentoDTO[$i]->getNumIdGrupoAcompanhamento()).'" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="'.PaginaSEI::getInstance()->getDiretorioImagensGlobal().'/consultar.gif" title="Consultar Grupo de Acompanhamento" alt="Consultar Grupo de Acompanhamento" class="infraImg" /></a>&nbsp;';
      }

      if ($bolAcaoAlterar){
        $strResultado .= '<a href="'.SessaoSEI::getInstance()->assinarLink('controlador.php?acao=grupo_acompanhamento_alterar&acao_origem='.$_GET['acao'].'&acao_retorno='.$_GET['acao'].'&id_grupo_acompanhamento='.$arrObjGrupoAcompanhamentoDTO[$i]->getNumIdGrupoAcompanhamento()).'" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="'.PaginaSEI::getInstance()->getDiretorioImagensGlobal().'/alterar.gif" title="Alterar Grupo de Acompanhamento" alt="Alterar Grupo de Acompanhamento" class="infraImg" /></a>&nbsp;';
      }
      ...
      if ($bolAcaoExcluir){
        $strResultado .= '<a href="'.PaginaSEI::getInstance()->montarAncora($strId).'" onclick="acaoExcluir(\''.$strId.'\',\''.$strDescricao.'\');" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="'.PaginaSEI::getInstance()->getDiretorioImagensGlobal().'/excluir.gif" title="Excluir Grupo de Acompanhamento" alt="Excluir Grupo de Acompanhamento" class="infraImg" /></a>&nbsp;';
      }
