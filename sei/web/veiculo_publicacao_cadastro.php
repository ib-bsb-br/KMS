...
#lblWebService {position:absolute;left:0%;top:0%;width:70%;}
#txtWebService {position:absolute;left:0%;top:12%;width:70%;}

#divSinExibirPesquisaInterna {position:absolute;left:0%;top:40%;}
#divSinFonteFeriados {position:absolute;left:0%;top:55%;}
#divSinPermiteExtraordinaria {position:absolute;left:0%;top:70%;visibility:hidden;}
...
function inicializar(){
  ...
  mostrarEsconderCamposVeiculoExterno();
...
function mostrarEsconderCamposVeiculoExterno(){
  if (document.getElementById('selStaTipo').value == 'E'){
    document.getElementById('divCamposVeiculoExterno').style.display = 'block';            
  }else{
    document.getElementById('divCamposVeiculoExterno').style.display = 'none';       
    document.getElementById('txtWebService').value = '';          
    document.getElementById('chkSinPermiteExtraordinaria').checked = false;    
  }
} 
...
?>
  ...
  <select id="selStaTipo" name="selStaTipo" onchange="mostrarEsconderCamposVeiculoExterno();" class="infraSelect" tabindex="<?=PaginaSEI::getInstance()->getProxTabDados()?>">
  <?=$strItensSelStaTipo?>
  </select>    
      
  </div>
  
  <div id="divCamposVeiculoExterno" class="infraAreaDados" style="height:15em;margin-top:2em;">

  <label id="lblWebService" for="txtWebService" accesskey="" class="infraLabelObrigatorio">Web Service:</label>
  <input type="text" id="txtWebService" name="txtWebService" class="infraText" value="<?=PaginaSEI::tratarHTML($objVeiculoPublicacaoDTO->getStrWebService());?>" onkeypress="return infraMascaraTexto(this,event,250);" maxlength="250" tabindex="<?=PaginaSEI::getInstance()->getProxTabDados()?>" />
  
  <div id="divSinExibirPesquisaInterna" class="infraDivCheckbox">
    <input type="checkbox" id="chkSinExibirPesquisaInterna" name="chkSinExibirPesquisaInterna" class="infraCheckbox" <?=PaginaSEI::getInstance()->setCheckbox($objVeiculoPublicacaoDTO->getStrSinExibirPesquisaInterna())?>  tabindex="<?=PaginaSEI::getInstance()->getProxTabDados()?>" />
    <label id="lblSinExibirPesquisaInterna" for="chkSinExibirPesquisaInterna" accesskey="" class="infraLabelCheckbox">Exibir as publicações enviadas para este veículo na pesquisa de publicações interna</label>
  </div>

  <div id="divSinFonteFeriados" class="infraDivCheckbox">
    <input type="checkbox" id="chkSinFonteFeriados" name="chkSinFonteFeriados" class="infraCheckbox" <?=PaginaSEI::getInstance()->setCheckbox($objVeiculoPublicacaoDTO->getStrSinFonteFeriados())?>  tabindex="<?=PaginaSEI::getInstance()->getProxTabDados()?>" />
    <label id="lblSinFonteFeriados" for="chkSinFonteFeriados" accesskey="" class="infraLabelCheckbox">Utilizar os  feriados cadastrados neste veículo como padrão para o sistema</label>
  </div>

  <div id="divSinPermiteExtraordinaria" class="infraDivCheckbox">
    <input type="checkbox" id="chkSinPermiteExtraordinaria" name="chkSinPermiteExtraordinaria" class="infraCheckbox" <?=PaginaSEI::getInstance()->setCheckbox($objVeiculoPublicacaoDTO->getStrSinPermiteExtraordinaria())?>  tabindex="<?=PaginaSEI::getInstance()->getProxTabDados()?>" />
    <label id="lblSinPermiteExtraordinaria" for="chkSinPermiteExtraordinaria" accesskey="" class="infraLabelCheckbox">Permite edição extraordinária</label>
  </div>  
