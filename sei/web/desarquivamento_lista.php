...
try {
  ...
  SessaoSEI::getInstance()->validarPermissao($_GET['acao']);
	...
	//PaginaSEI::getInstance()->salvarCamposPost(array('selUnidade','selUsuario'));  
  ...
  switch($_GET['acao']){
    ...
    case 'arquivamento_desarquivar':
      ...
      try{
        ...
        $objArquivamentoDTO->setNumIdUsuario($_POST['selUsuario']);
        $objArquivamentoDTO->setStrSenha($_POST['pwdSenha']);
        ...
        $objArquivamentoRN->desarquivarRN1147($objArquivamentoDTO);
  ...
  $objUnidadeDTO->setNumIdUnidade($_POST['selUnidade']);
  ...
  if ($numRegistros > 0){
    ...
    for($i = 0;$i < $numRegistros; $i++){  
      ...
      if ($bolAcaoProtocoloDesarquivar && $arrObjArquivamentoDTO[$i]->getStrStaArquivamento()==ArquivamentoRN::$TA_SOLICITADO_DESARQUIVAMENTO && $arrObjArquivamentoDTO[$i]->getNumIdUnidadeLocalizador()==SessaoSEI::getInstance()->getNumIdUnidadeAtual()){
				$strResultado .= '<a href="#ID-'.$arrObjArquivamentoDTO[$i]->getDblIdProtocolo().'"  onclick="acaoDesarquivar(\''.$arrObjArquivamentoDTO[$i]->getDblIdProtocolo().'\',\''.$arrObjArquivamentoDTO[$i]->getStrProtocoloFormatadoDocumento().'\');" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="'.PaginaSEI::getInstance()->getDiretorioImagensGlobal().'/marcar.gif" title="Desarquivar Documento" alt="Desarquivar Documento" class="infraImg" /></a>&nbsp;';
      }
      ...
      if ($bolAcaoProtocoloCancelarSolicitacaoDesarquivamento && $arrObjArquivamentoDTO[$i]->getStrStaArquivamento()==ArquivamentoRN::$TA_SOLICITADO_DESARQUIVAMENTO && $arrObjArquivamentoDTO[$i]->getNumIdUnidadeLocalizador()==SessaoSEI::getInstance()->getNumIdUnidadeAtual()){
				$strResultado .= '<a href="#ID-'.$arrObjArquivamentoDTO[$i]->getDblIdProtocolo().'"  onclick="acaoCancelarSolicitacao(\''.$arrObjArquivamentoDTO[$i]->getDblIdProtocolo().'\',\''.$arrObjArquivamentoDTO[$i]->getStrProtocoloFormatadoDocumento().'\');" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="'.PaginaSEI::getInstance()->getDiretorioImagensGlobal().'/menos.gif" title="Cancelar Solicitação de Desarquivamento" alt="Cancelar Solicitação de Desarquivamento" class="infraImg" /></a>&nbsp;';
      }
	    $strResultado .= '</td>';
  ...
  $strItensSelUnidade = ProtocoloINT::montarSelectUnidadesSolicitantesDesarquivamento('null','&nbsp;',$_POST['selUnidade']);
