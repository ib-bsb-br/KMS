...
try {
  ...
  if ($numRegistros){
    ...
    foreach ($SEI_MODULOS as $seiModulo) {
      if (count($arrObjAcessoExternoAPI)){
    ...
    foreach($arrObjAcessoExternoDTO as $objAcessoExternoDTO){
      ...
      $strLinkProcedimento = SessaoSEIExterna::getInstance()->assinarLink('processo_acesso_externo_consulta.php?id_acesso_externo='.$objAcessoExternoDTO->getNumIdAcessoExterno().'&id_procedimento='.$objProcedimentoDTO->getDblIdProcedimento());
      ...
      if (InfraData::compararDatas(date('d/m/Y'), $objAcessoExternoDTO->getDtaValidade()) < 0){
        $strResultado .= '<td align="center"><a href="javascript:void(0);" onclick="alert(\'Este acesso externo expirou em '.$objAcessoExternoDTO->getDtaValidade().'.\');" alt="' . PaginaSEIExterna::tratarHTML($objProcedimentoDTO->getStrNomeTipoProcedimento()) . '" title="' . PaginaSEIExterna::tratarHTML($objProcedimentoDTO->getStrNomeTipoProcedimento()) . '" class="ancoraPadraoPreta">' . PaginaSEIExterna::tratarHTML($objProcedimentoDTO->getStrProtocoloProcedimentoFormatado()) . '</a></td>';
      }else {
        if ($objAcessoExternoDTO->getStrSinProcesso() == 'S') {
          $strResultado .= '<td align="center"><a href="javascript:void(0);" onclick="window.open(\'' . $strLinkProcedimento . '\');" alt="' . PaginaSEIExterna::tratarHTML($objProcedimentoDTO->getStrNomeTipoProcedimento()) . '" title="' . PaginaSEIExterna::tratarHTML($objProcedimentoDTO->getStrNomeTipoProcedimento()) . '" class="ancoraPadraoAzul">' . PaginaSEIExterna::tratarHTML($objProcedimentoDTO->getStrProtocoloProcedimentoFormatado()) . '</a></td>';
        } else {
          $strResultado .= '<td align="center"><a href="javascript:void(0);" onclick="alert(\'Sem acesso à íntegra do processo.\');" alt="' . PaginaSEIExterna::tratarHTML($objProcedimentoDTO->getStrNomeTipoProcedimento()) . '" title="' . PaginaSEIExterna::tratarHTML($objProcedimentoDTO->getStrNomeTipoProcedimento()) . '" class="ancoraPadraoPreta">' . PaginaSEIExterna::tratarHTML($objProcedimentoDTO->getStrProtocoloProcedimentoFormatado()) . '</a></td>';
        }
      }
