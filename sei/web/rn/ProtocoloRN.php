  ...
  protected function alterarRN0203Controlado(ProtocoloDTO $objProtocoloDTO){
    try {
      ...
      if ($objProtocoloDTO->isSetStrProtocoloFormatado()){
        if ($objProtocoloDTO->getStrProtocoloFormatado()!=$objProtocoloDTOBanco->getStrProtocoloFormatado()){
          $objInfraException->adicionarValidacao('Número formatado do protocolo não pode ser alterado.');
        }
      }else{
      ...
      if ($objProtocoloDTO->isSetArrObjParticipanteDTO()){
        ...
        foreach($arrRemocao as $participanteRemover){
          if ($participanteRemover->getNumIdUnidade()<>SessaoSEI::getInstance()->getNumIdUnidadeAtual()){
            ...
            $objUnidadeDTO->setNumIdUnidade($participanteRemover->getNumIdUnidade());
            ...
            $objInfraException->lancarValidacao('O '.$strPapel.' "'.$participanteRemover->getStrNomeContato().'" não pode ser excluído porque foi adicionado por outra unidade ('.$objUnidadeDTO->getStrSigla().').');
          }
        }
        ...
        foreach($arrAssuntosAntigos as $numIdAssuntoAntigoProxy => $assuntoAntigo){
          ...
          if (!isset($arrAssuntosNovos[$numIdAssuntoAntigo])){
            if (!in_array($numIdAssuntoAntigo,$arrAssuntosPermitidosExclusao)){
              ...
              $objUnidadeDTO = new UnidadeDTO();
              $objUnidadeDTO->retStrSigla();
              $objUnidadeDTO->setNumIdUnidade($assuntoAntigo->getNumIdUnidade());
              $objUnidadeDTO = $objUnidadeRN->consultarRN0125($objUnidadeDTO);
              $objInfraException->lancarValidacao('O assunto "'.$assuntoAntigo->getStrCodigoEstruturadoAssunto().'" não pode ser excluído porque foi adicionado por outra unidade ('.$objUnidadeDTO->getStrSigla().').');
            }else {
