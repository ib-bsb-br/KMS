 ...
 */
  ...
  protected function pesquisarConectado(RelGrupoContatoDTO $objRelGrupoContatoDTO) {
    try {
      ...
      if ($objRelGrupoContatoDTO->isSetStrPalavrasPesquisa()){

        if (!InfraString::isBolVazia($objRelGrupoContatoDTO->getStrPalavrasPesquisa())){

          $strPalavrasPesquisa = InfraString::prepararIndexacao($objRelGrupoContatoDTO->getStrPalavrasPesquisa(),false);
          ...
          if ($numPalavrasPesquisa){
            ...
            if ($numPalavrasPesquisa==1){
              $objRelGrupoContatoDTO->setStrIdxContatoContato($arrPalavrasPesquisa[0],InfraDTO::$OPER_LIKE);
            }else{
              $a = array_fill(0,$numPalavrasPesquisa,'IdxContatoContato');
              $b = array_fill(0,$numPalavrasPesquisa,InfraDTO::$OPER_LIKE);
              $d = array_fill(0,$numPalavrasPesquisa-1,InfraDTO::$OPER_LOGICO_AND);
              $objRelGrupoContatoDTO->adicionarCriterio($a,$b,$arrPalavrasPesquisa,$d);
            }
          }
        }
      }
