...
class ProcedimentoRN extends InfraRN {
  ...
  protected function reabrirRN0966Controlado(ReabrirProcessoDTO $objReabrirProcessoDTO){
    try {
      ...
      if(count($arrObjAtividadeDTO)==0){
      	if ($objProcedimentoDTO->getStrStaNivelAcessoGlobalProtocolo()!=ProtocoloRN::$NA_SIGILOSO){
	        $objInfraException->lancarValidacao('Processo não tramitou pela unidade atual.');
      	}else{
      		$objInfraException->lancarValidacao('Processo não encontrado para reabertura.');
      	}
      }else{ 
      	foreach($arrObjAtividadeDTO as $objAtividadeDTO){
  ...
  protected function removerRelacionamentoProcedimentoRN1021Controlado(RelProtocoloProtocoloDTO $objRelProtocoloProtocoloRecebidoDTO){
    try {
      ...
      $objRelProtocoloProtocoloDTOConsulta->setDblIdProtocolo1($objRelProtocoloProtocoloRecebidoDTO->getDblIdProtocolo1());
      $objRelProtocoloProtocoloDTOConsulta->setDblIdProtocolo2($objRelProtocoloProtocoloRecebidoDTO->getDblIdProtocolo2());
      $objRelProtocoloProtocoloDTOConsulta->setStrStaAssociacao(RelProtocoloProtocoloRN::$TA_PROCEDIMENTO_RELACIONADO);

 		  $objRelProtocoloProtocoloDTO = $objRelProtocoloProtocoloRN->consultarRN0841($objRelProtocoloProtocoloDTOConsulta);
 		   
			if ($objRelProtocoloProtocoloDTO==null){
        $objRelProtocoloProtocoloDTOConsulta->setDblIdProtocolo1($objRelProtocoloProtocoloRecebidoDTO->getDblIdProtocolo2());
        $objRelProtocoloProtocoloDTOConsulta->setDblIdProtocolo2($objRelProtocoloProtocoloRecebidoDTO->getDblIdProtocolo1());
	 		  $objRelProtocoloProtocoloDTO = $objRelProtocoloProtocoloRN->consultarRN0841($objRelProtocoloProtocoloDTOConsulta);
			}
      ...
      $objRelProtocoloProtocoloRN->excluirRN0842(array($objRelProtocoloProtocoloDTO));
