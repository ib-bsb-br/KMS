	...
	public function consultarHTMLDocumentoRN1204(DocumentoDTO $parObjDocumentoDTO)
  {
    ...
    try {
      ...
      if (ConfiguracaoSEI::getInstance()->getValor('Edoc', 'Servidor', false) == null){
        ...
        $objDocumentoDTO->setStrConteudoAssinatura(null, InfraDTO::$OPER_DIFERENTE);
      ...
      SeiINT::validarXss($ret, 'eDoc '.$parObjDocumentoDTO->getDblIdDocumentoEdoc());
...
//          return false;
    ...
    if (count($rs) != count($arrObjAssinaturaDTO)) {
      $strErro = 'Divergência na quantidade de assinaturas  ['.count($rs).','.count($arrObjAssinaturaDTO).'] ' . $strDocumento . " ** Ignorado **\n";
      echo $strErro;
    }
