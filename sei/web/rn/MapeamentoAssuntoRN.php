 ...
 */
  ...
  protected function pesquisarConectado(MapeamentoAssuntoDTO $parObjMapeamentoAssuntoDTO){
    try {
      ...
      if ($parObjMapeamentoAssuntoDTO->isSetStrPalavrasPesquisa()){
        if (trim($parObjMapeamentoAssuntoDTO->getStrPalavrasPesquisa())!=''){
          $strPalavrasPesquisa = InfraString::prepararIndexacao($parObjMapeamentoAssuntoDTO->getStrPalavrasPesquisa());
          ...
          if ($numPalavrasPesquisa){
            ...
            if ($numPalavrasPesquisa==1){
              $objAssuntoProxyDTO->setStrIdxAssuntoAssunto($arrPalavrasPesquisa[0],InfraDTO::$OPER_LIKE);
            }else{
              $a = array_fill(0,$numPalavrasPesquisa,'IdxAssuntoAssunto');
              $b = array_fill(0,$numPalavrasPesquisa,InfraDTO::$OPER_LIKE);
              $d = array_fill(0,$numPalavrasPesquisa-1,InfraDTO::$OPER_LOGICO_AND);
              $objAssuntoProxyDTO->adicionarCriterio($a,$b,$arrPalavrasPesquisa,$d);
            }
          }
        }
      }
