...
class PublicacaoRN extends InfraRN {
	...
	private function obterEstadoRN1102($arrObjPublicacaoDTO) {
		try{
			...
			foreach($arrObjPublicacaoDTO as $objPublicacaoDTO){
				...
				}else if($objPublicacaoDTO->getStrStaTipoVeiculoPublicacao() == VeiculoPublicacaoRN::$TV_EXTERNO){
      ...
      $objPublicacaoBancoDTO->retStrStaEstado();
        ...
        $objInfraException->lancarValidacao('Não é possível alterar um agendamento que já foi publicado.');
			...
			if ($objPublicacaoDTO->isSetDblIdDocumento()){
				if($objPublicacaoDTO->getDblIdDocumento() != $objPublicacaoBancoDTO->getDblIdDocumento()){
					$objInfraException->lancarValidacao('Não é possível alterar o documento do agendamento.');
				}
			}else{
				$objPublicacaoDTO->setDblIdDocumento($objPublicacaoBancoDTO->getDblIdDocumento());
			}

			if (!$objPublicacaoDTO->isSetStrStaMotivo()){
				$objPublicacaoDTO->setStrStaMotivo($objPublicacaoBancoDTO->getStrStaMotivo());
			}

			if (!$objPublicacaoDTO->isSetNumIdVeiculoPublicacao()){
			  ...
			  $objPublicacaoDTO->setStrNomeVeiculoPublicacao($objVeiculoPublicacaoDTO->getStrNome());
			}

			if (!$objPublicacaoDTO->isSetDtaDisponibilizacao()){
			...
			if ($objPublicacaoDTO->isSetDtaPublicacao()){
				if($objPublicacaoDTO->getDtaPublicacao() != $objPublicacaoBancoDTO->getDtaPublicacao()){
			...
			if ($objPublicacaoBancoDTO->getDtaDisponibilizacao() != $objPublicacaoDTO->getDtaDisponibilizacao() ||
			    ($objPublicacaoBancoDTO->getStrStaTipoVeiculoPublicacao() == VeiculoPublicacaoRN::$TV_EXTERNO
  			   && $objPublicacaoDTO->getStrStaTipoVeiculoPublicacao() == VeiculoPublicacaoRN::$TV_INTERNO)){
  ...
  protected function alterarMetadadosPublicadosControlado(PublicacaoDTO $parObjPublicacaoDTO){
    try {

    	//filtra campos
    	$objPublicacaoDTO = new PublicacaoDTO();
      $objPublicacaoDTO->setStrResumo($parObjPublicacaoDTO->getStrResumo());
      $objPublicacaoDTO->setNumIdVeiculoIO($parObjPublicacaoDTO->getNumIdVeiculoIO());
      $objPublicacaoDTO->setDtaPublicacaoIO($parObjPublicacaoDTO->getDtaPublicacaoIO());
      $objPublicacaoDTO->setNumIdSecaoIO($parObjPublicacaoDTO->getNumIdSecaoIO());
      $objPublicacaoDTO->setStrPaginaIO($parObjPublicacaoDTO->getStrPaginaIO());
      $objPublicacaoDTO->setNumIdPublicacao($parObjPublicacaoDTO->getNumIdPublicacao());
      ...
      if ($objPublicacaoDTO->getStrStaEstado()==self::$TE_PUBLICADO) {

        $objIndexacaoDTO = new IndexacaoDTO();
        $objIndexacaoDTO->setArrObjPublicacaoDTO(array($objPublicacaoDTO));
      ...
      throw new InfraException('Erro alterando dados de Publicação.',$e);
    }
  }
