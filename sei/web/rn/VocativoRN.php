    ...
    try {
      ...
      for ($i=0;$i<count($arrObjVocativoDTO);$i++){
        ...
        $objCargoDTO->setStrSinAtivo('S');
      	if ($objCargoRN->contarRN0304($objCargoDTO)){
      		$objInfraException->lancarValidacao('Existem contatos utilizando este vocativo.');
      	}

        $objCargoDTO->setStrSinAtivo('N');
 	      if ($objCargoRN->contarRN0304($objCargoDTO)){
          $objInfraException->lancarValidacao('Existem contatos inativos utilizando este vocativo.'); 	        
 	      }
      }
