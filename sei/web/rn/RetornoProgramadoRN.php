  ...
  protected function excluirControlado($arrObjRetornoProgramadoDTO){
    try {
      ...
      for($i=0;$i<count($arrObjRetornoProgramadoDTO);$i++) {
        ...
        if ($dto != null) {
          $objInfraException->adicionarValidacao('Processo '.$dto->getStrProtocoloFormatadoAtividadeEnvio().' já foi devolvido pela unidade '.$dto->getStrSiglaUnidadeAtividadeEnvio().'.');
        }
      }
      $objInfraException->lancarValidacoes();
