  ...
  protected function excluirRN0006InternoControlado(DocumentoDTO $parObjDocumentoDTO){
    try {
      ...
      if($objDocumentoDTO->getNumIdUnidadeGeradoraProtocolo()!= SessaoSEI::getInstance()->getNumIdUnidadeAtual()){
        $objInfraException->adicionarValidacao('Documento '.$objDocumentoDTO->getStrProtocoloDocumentoFormatado().' somente pode ser excluído pela unidade geradora.');
      }

      if ($objDocumentoDTO->getStrStaDocumento()==DocumentoRN::$TD_FORMULARIO_AUTOMATICO){
        $objInfraException->adicionarValidacao('Formulário '.$objDocumentoDTO->getStrProtocoloDocumentoFormatado().' não pode ser excluído.');
      }
      ...
      if ($objRelProtocoloProtocoloRN->consultarRN0841($objRelProtocoloProtocoloDTO) != null){
        $objInfraException->adicionarValidacao('Não foi possível excluir o documento '.$objDocumentoDTO->getStrProtocoloDocumentoFormatado().' porque ele foi movimentado entre processos.');
      }
      ...
      if (($numCircular = $objRelProtocoloProtocoloRN->contarRN0843($objRelProtocoloProtocoloDTO))){
        $objInfraException->adicionarValidacao('Não foi possível excluir o documento '.$objDocumentoDTO->getStrProtocoloDocumentoFormatado().' porque ele é base para '.($numCircular==1?'um documento circular':$numCircular.' documentos circulares').'.');
      }
      ...
      if ($objDocumentoDTO->getStrSinBloqueado()=='S'){
        $objInfraException->adicionarValidacao('Não é mais possível excluir o documento '.$objDocumentoDTO->getStrProtocoloDocumentoFormatado().'.');
      }
      ...
      if ($objDocumentoDTO->getStrStaNivelAcessoGlobalProtocolo() == ProtocoloRN::$NA_SIGILOSO) {
        ...
        if ($objAtributoAndamentoRN->consultarRN1366($objAtributoAndamentoDTO) != null){
          $objInfraException->adicionarValidacao('Não foi possível excluir o documento '.$objDocumentoDTO->getStrProtocoloDocumentoFormatado().' porque existe Credencial para Assinatura ativa.');
        }
      }
  ...
  protected function obterHashDocumentoAssinaturaConectado(AssinaturaDTO $parObjAssinaturaDTO){
    try{
      ...
      if ($objDocumentoDTO->getStrStaProtocoloProtocolo()==ProtocoloRN::$TP_DOCUMENTO_GERADO){
        ...
        $ret = hash('sha256', $objDocumentoDTO->getStrConteudoAssinatura(), true);
        ...
        $ret = hash_file('sha256', $objAnexoRN->obterLocalizacao($objAnexoDTO), true);
      }
    ...
    try{
      ...
      foreach($arrIdDocumentos as $dblIdDocumento){
        ...
        if ($objDocumentoDTO->getStrStaProtocoloProtocolo() == ProtocoloRN::$TP_DOCUMENTO_GERADO){
          ...
          $arrComandoExecucao[] = DIR_SEI_BIN.'/wkhtmltopdf-amd64 --quiet --title processo-'.InfraUtil::retirarFormatacao($objDocumentoDTO->getStrProtocoloProcedimentoFormatado(),false) .' ' .$strArquivoHtmlTemp.' ' .$strArquivoPdfParcial .' 2>&1';
        }else if ($objDocumentoDTO->getStrStaProtocoloProtocolo()==ProtocoloRN::$TP_DOCUMENTO_RECEBIDO){
          $objAnexoDTO = new AnexoDTO();
          $objAnexoDTO->retNumIdAnexo();
          $objAnexoDTO->retStrNome();
          ...
          if (InfraUtil::getStrMimeType($objAnexoDTO->getStrNome()) == 'application/pdf' || InfraUtil::getStrMimeType($objAnexoDTO->getStrNome()) == 'text/html' || InfraUtil::getStrMimeType($objAnexoDTO->getStrNome()) == 'text/plain' || $this->ehUmaExtensaoDeImagemPermitida($objAnexoDTO->getStrNome()) || $this->processarArquivoOpenOffice($objAnexoDTO->getStrNome())){
            if ($objAnexoDTO==null){
              ...
              if (InfraUtil::getStrMimeType($objAnexoDTO->getStrNome()) == 'application/pdf'){
                ...
                $strArquivoPdfParcial = DIR_SEI_TEMP.'/'.$objAnexoRN->gerarNomeArquivoTemporario('-parte'.$numParteArquivoPdf++.'.pdf');
                $arrArquivoPdfParcial[] = $strArquivoPdfParcial;
                $arrComandoExecucao[] = DIR_SEI_BIN.'/wkhtmltopdf-amd64 --quiet --title processo-'.InfraUtil::retirarFormatacao($objDocumentoDTO->getStrProtocoloProcedimentoFormatado(),false) .' ' .$strCaminhoCompletoArquivoTxt  .' ' .$strArquivoPdfParcial .' 2>&1';
