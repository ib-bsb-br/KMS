...
class AssuntoRN extends InfraRN {
  ...
  protected function cadastrarRN0259Controlado(AssuntoDTO $objAssuntoDTO) {
    try{
      ...
      $this->montarIndexacaoRN0505($ret);
  ...
  protected function alterarRN0260Controlado(AssuntoDTO $objAssuntoDTO){
    try {
      ...
      $this->montarIndexacaoRN0505($objAssuntoDTO);      
  ...
  protected function pesquisarRN0246Conectado(AssuntoDTO $objAssuntoDTO){
    try {
      ...
      //$objInfraException->lancarValidacoes();
  		...
  		if ($objAssuntoDTO->isSetStrPalavrasPesquisa()){
  		  if (trim($objAssuntoDTO->getStrPalavrasPesquisa())!=''){
    			$strPalavrasPesquisa = InfraString::prepararIndexacao($objAssuntoDTO->getStrPalavrasPesquisa());
    			...
    			if ($numPalavrasPesquisa){
      			...
      			if ($numPalavrasPesquisa==1){
      				$objAssuntoDTO->setStrIdxAssunto($arrPalavrasPesquisa[0],InfraDTO::$OPER_LIKE);
      			}else{
      				$a = array_fill(0,$numPalavrasPesquisa,'IdxAssunto');
      				$b = array_fill(0,$numPalavrasPesquisa,InfraDTO::$OPER_LIKE);
      				$d = array_fill(0,$numPalavrasPesquisa-1,InfraDTO::$OPER_LOGICO_AND);
      				$objAssuntoDTO->adicionarCriterio($a,$b,$arrPalavrasPesquisa,$d);
      			}
     			}
  		  }			
  		}
  ...
  protected function montarIndexacaoRN0505Controlado(AssuntoDTO $objAssuntoDTO){
  	$dto = new AssuntoDTO();
  	$dto->retTodos(true);
    ...
    $objAssuntoDTO->setNumIdAssunto($dto->getNumIdAssunto());
  	
  	$strIndexacao = $dto->getStrCodigoEstruturado();
  	$strIndexacao .= ' '.InfraString::prepararIndexacao($dto->getStrCodigoEstruturado());
  	$strIndexacao .= ' '.InfraString::prepararIndexacao($dto->getStrDescricao());
  	$strIndexacao .= ' '.InfraString::prepararIndexacao($dto->getStrObservacao());

    $objAssuntoDTO->setStrIdxAssunto($strIndexacao);
