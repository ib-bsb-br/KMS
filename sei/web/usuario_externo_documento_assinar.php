  ...
  switch($_GET['acao']){
    ...
    case 'usuario_externo_documento_assinar':
      ...
      if (count($arrObjAcessoExternoDTO)!=1) {
        throw new InfraException('Documento não está disponível.');
      }
...
//<script type="text/javascript">
  ...
  function assinar(){
      infraAbrirJanela('<?=$strLinkAssinatura;?>','janelaAssinatura',450,300,'location=0,status=1,resizable=1,scrollbars=1');
  }
...
<? if ($objDocumentoDTO != null && !$bolFlagAssinou && $objDocumentoDTO->getStrStaEstadoProtocolo()!=ProtocoloRN::$TE_DOCUMENTO_CANCELADO){ ?>
    <div id="divAcoes">
      <button id="btnAssinar" onclick="assinar()" style="">
        <img id="imgAssinatura" src="imagens/sei_assinar_pequeno.gif" alt="Assinar Documento" title="Assinar Documento" style="float: left;">
        <span style="padding: 5px;">Assinar</span>
      </button>
    </div>
<? } ?>
</div>
