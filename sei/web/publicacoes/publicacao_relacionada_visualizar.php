 ...
 * 12/02/2008 - ALTERADO POR mkr@trf4.gov.br - ADICIONADA CONSULTA DE NÃO-PUBLICADOS NO DIÁRIO ELETRÔNICO PARA ADMINISTRADORES
 */
try {
    ...
    case 'publicacao_relacionada_visualizar':
      ...
      if ($numRegistros){
        ...
        foreach($arrObjPublicacaoDTO as $objPublicacaoDTO){
          ...
          $strResultado .= PublicacaoINT::montarDadosImprensaNacional($objPublicacaoDTO->getStrSiglaVeiculoImprensaNacional(),
                                                                      $objPublicacaoDTO->getStrDescricaoVeiculoImprensaNacional(),
                                                                      $objPublicacaoDTO->getDtaPublicacaoIO(),
                                                                      $objPublicacaoDTO->getStrNomeSecaoImprensaNacional(),
                                                                      $objPublicacaoDTO->getStrPaginaIO());
          $strResultado .= '</td>';
        }
        $strResultado .= '</table>';
      }
