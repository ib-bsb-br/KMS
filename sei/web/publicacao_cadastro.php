...
try {
  ...
  switch($_GET['acao']){
    case 'publicacao_agendar':
      ...
      $arrComandos[] = '<button type="submit" accesskey="S" name="sbmCadastrarPublicacao" value="Salvar" class="infraButton"><span class="infraTeclaAtalho">S</span>alvar</button>';
    ...
    case 'publicacao_alterar_agendamento':
      ...
      $arrComandos[] = '<button type="submit" accesskey="S" name="sbmAlterarPublicacao" value="Salvar" class="infraButton"><span class="infraTeclaAtalho">S</span>alvar</button>';
      ...
      if (isset($_GET['id_publicacao'])){
        ...
        $objPublicacaoDTOBanco = new PublicacaoDTO();
        $objPublicacaoDTOBanco->retStrStaEstado();
        $objPublicacaoDTOBanco->retStrStaMotivo();
        $objPublicacaoDTOBanco->retNumIdVeiculoPublicacao();
        $objPublicacaoDTOBanco->retDtaDisponibilizacao();
        $objPublicacaoDTOBanco->setNumIdPublicacao($_POST['hdnIdPublicacao']);

        $objPublicacaoDTOBanco = $objPublicacaoRN->consultarRN1044($objPublicacaoDTOBanco);
        if ($objPublicacaoDTOBanco==null){
          throw new InfraException("Registro não encontrado.");
        }

        $objPublicacaoDTO->setStrStaEstado($objPublicacaoDTOBanco->getStrStaEstado());

        if ($objPublicacaoDTO->getStrStaEstado()==PublicacaoRN::$TE_PUBLICADO){
          $objPublicacaoDTO->setStrStaMotivo($objPublicacaoDTOBanco->getStrStaMotivo());
          $objPublicacaoDTO->setNumIdVeiculoPublicacao($objPublicacaoDTOBanco->getNumIdVeiculoPublicacao());
          $objPublicacaoDTO->setDtaDisponibilizacao($objPublicacaoDTOBanco->getDtaDisponibilizacao());
        }
      }
      ...
      if (isset($_POST['sbmAlterarPublicacao'])) {
        ...
        try{
          ...
          if ($_POST['hdnStaEstado']==PublicacaoRN::$TE_AGENDADO && $objPublicacaoDTOBanco->getStrStaEstado()==PublicacaoRN::$TE_PUBLICADO &&
              ($_POST['selStaMotivo']!=$objPublicacaoDTOBanco->getStrStaMotivo() ||
               $_POST['selVeiculoPublicacao']!=$objPublicacaoDTOBanco->getNumIdVeiculoPublicacao() ||
               $_POST['txtDisponibilizacao']!=$objPublicacaoDTOBanco->getDtaDisponibilizacao())){

            $objInfraException = new InfraException();
            $objInfraException->lancarValidacao('Não é possível realizar as alterações solicitadas porque o estado da publicação mudou desde a última visualização.');
            ...
            if ($objPublicacaoDTO->getStrStaEstado() == PublicacaoRN::$TE_AGENDADO) {
              ...
              $objPublicacaoRN->alterarMetadadosPublicados($objPublicacaoDTO);
            }
...
?>
	<div id="divDocumento" class="infraAreaDados" style="height:18em;">
	  ...
	  <label id="lblVeiculoPublicacao" for="selVeiculoPublicacao" accesskey="" class="infraLabelObrigatorio">Veiculo:</label>
	  <select id="selVeiculoPublicacao" name="selVeiculoPublicacao" onchange="trocarDataDisponibilizacao();" class="infraSelect" tabindex="<?=PaginaSEI::getInstance()->getProxTabDados()?>" <?=$strDesabilitar?> >
