  ...
  if ($numRegistros > 0){
    ...
    for($i = 0;$i < $numRegistros; $i++){
      ...
      if($bolAcaoExcluirRelacionamento && $arrObjRelProtocoloProtocoloDTO[$i]->getNumIdUnidade()==SessaoSEI::getInstance()->getNumIdUnidadeAtual()){
        $strResultado .= '<a href="#ID-'.$strId.'"  onclick="acaoRemover(\''.$strId.'\');" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="imagens/remover_vinculo_procedimento.gif" title="Remover Relacionamento" alt="Remover Relacionamento" class="infraImg" /></a>&nbsp;';
      }
