...
try {
  ...
  if ($_GET['acao']=='documento_alterar' || $_GET['acao']=='documento_alterar_recebido' || $_GET['acao']=='publicacao_gerar_relacionada'){
    ...
    $arrInteressadosNegados = array();
    $arrDestinatariosNegados = array();
    foreach($arrObjParticipanteDTO as $objParticipanteDTO){
      if ($objParticipanteDTO->getStrStaParticipacao()==ParticipanteRN::$TP_INTERESSADO){
        $arrInteressadosNegados[] = $objParticipanteDTO;
      }else if ($objParticipanteDTO->getStrStaParticipacao()==ParticipanteRN::$TP_DESTINATARIO){
        $arrDestinatariosNegados[] = $objParticipanteDTO;
      }
    }
    $strInteressadosNegados = implode(',',InfraArray::converterArrInfraDTO($arrInteressadosNegados,'IdContato'));
    $strDestinatariosNegados = implode(',',InfraArray::converterArrInfraDTO($arrDestinatariosNegados,'IdContato'));
  }
  ...
  ProtocoloINT::montarNivelAcesso(array($numIdTipoProcedimento),
      $objProtocoloDTO,
      ($_GET['acao']=='documento_consultar' || $_GET['acao']=='documento_consultar_recebido'),
      $strCssNivelAcesso,
      $strHtmlNivelAcesso,
  ...
  //ANEXOS
  $bolAlteracaoAnexoPermitida = $objDocumentoDTO->getStrSinBloqueado()=='N' &&
                                $_GET['acao']!='documento_consultar' &&
...
?>
  ...
  function inicializar(){
    ...
    objLupaDestinatarios.processarRemocao = function(itens){
      for(var i=0;i < itens.length;i++){
        if (infraInArray(itens[i].value,[<?=$strDestinatariosNegados?>])){
          alert('Destinatário \"' + itens[i].text + '\" não pode ser removido porque não foi adicionado por esta unidade.');
          return false;
        }
      }
      return true;
