...
try {
  ...
  if (isset($_POST['txtTextoPesquisa'])){
  	$objContatoDTO->setStrPalavrasPesquisa(trim($_POST['txtTextoPesquisa']));
 	}
  ...
  PaginaSEI::getInstance()->prepararPaginacao($objContatoDTO);
...
?>
#lblTextoPesquisa {position:absolute;left:0%;top:0%;}
#txtTextoPesquisa {position:absolute;left:0%;top:20%;width:50%;}
...
<form id="frmContatoRelatorioTemporarios" method="post" action="<?=SessaoSEI::getInstance()->assinarLink('controlador.php?acao='.$_GET['acao'].'&acao_origem='.$_GET['acao'])?>">
  ...
  PaginaSEI::getInstance()->abrirAreaDados('10em');
	?>
  <label id="lblTextoPesquisa" class="infraLabel" tabindex="<?=PaginaSEI::getInstance()->getProxTabDados()?>">Texto para Pesquisa:</label>
  <input type="text" name="txtTextoPesquisa" id="txtTextoPesquisa" onkeyup="return tratarEnter(event);" class="infraText" value="<?=PaginaSEI::tratarHTML($_POST['txtTextoPesquisa'])?>"/>
