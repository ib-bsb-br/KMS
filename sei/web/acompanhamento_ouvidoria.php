...
try {
  ...
  if (count($arrNumIdTiposProcedimento)) {
  ...
  if (isset($_POST['hdnTipo'])){
    ...
    if ($_POST['hdnTipo']=='P'){
      ...
      $numRegistros = count($arrObjAcompanhamentoOuvidoriaDTO);
      ...
      if ($numRegistros >0){
        ...
        $strResultado .= '<th class="infraTh" width="15%">'.PaginaSEI::getInstance()->getThOrdenacao($objAcompanhamentoOuvidoriaDTO,'Destino','SiglaUnidade',$arrObjAcompanhamentoOuvidoriaDTO).'</th>'."\n";
...
#lblUnidade {position:absolute;left:0%;top:65%;width:70%;}
#txtUnidade {position:absolute;left:10%;top:63%;width:70%;}

#lblTipoProcedimento {position:absolute;left:0%;top:86%;width:9%;}
...
function inicializar(){
  ...
  objAutoCompletarUnidade.selecionar('<?=$strIdUnidade?>','<?=PaginaSEI::getInstance()->formatarParametrosJavaScript($strNomeUnidade)?>');
...
function abrirDetalhe(link){
 infraAbrirJanela(link,'janelaAcompanhamentoOuvidoriaDetalhe',850,550,'location=0,status=1,resizable=1,scrollbars=1');
}
...
<form id="frmAcompanhamentoOuvidoria"  method="post" onsubmit="return onSubmitForm();" action="<?=SessaoSEI::getInstance()->assinarLink('controlador.php?acao='.$_GET['acao'].'&acao_origem='.$_GET['acao'])?>">
  ...
  ?>
    ...
    <select multiple id="selTipoProcedimento" name="selTipoProcedimento[]" class="infraSelect multipleSelect" tabindex="<?=PaginaSEI::getInstance()->getProxTabDados()?>">
      <?=$strOptionsTipoProcedimento;?>
    </select>
